...
function attachDefaultAiService(deps) {
  ...
  function installedVendors(found, ws, actorInfo) {
    if (!found || !found.ctx || typeof found.ctx.getVendorModelAvailability !== "function") return [];
    var choices = found.ctx.getVendorModelAvailability(catalogSocket(ws, actorInfo)) || [];
  ...
  async function resolve(ws, actorInfo, preferenceOverride) {
    ...
    if (!found || !found.ctx || typeof found.ctx.getVendorModelCatalog !== "function") {
      return { ready: false, accountAvailable: true, error: "Clay Default AI is unavailable.", installedVendors: installedVendors(found, ws, actorInfo) };
    }
    var installed = installedVendors(found, ws, actorInfo);
    var result;
    try {
