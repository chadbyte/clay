  ...
  function handleSessionsMessage(ws, msg) {
    ...
    if (msg.type === "new_session") {
      if (msg.scheduleInterview === true) {
        ...
        var interviewVendor = typeof msg.vendor === "string" ? msg.vendor : "";
        var installed = sm.installedVendors || [];
        var knownModels = sm.modelsByVendor && sm.modelsByVendor[interviewVendor] || [];
        var requestedModel = typeof msg.model === "string" ? msg.model : "";
        var requestedEffort = typeof msg.effort === "string" ? msg.effort : "";
        var vendorInfo = yoke.getVendorInfo(interviewVendor);
        var engineError = "";
        var sourceMatches = !!sourceSession && Number(msg.sourceSessionId) === Number(sourceSession.localId);
        if (!sourceMatches || msg.projectSlug !== slug || !ctx.canCreateScheduledTaskInterview || !ctx.canCreateScheduledTaskInterview(ws, sourceSession)) engineError = "Scheduled Tasks is unavailable in the current project session.";
        else if (!interviewVendor || installed.indexOf(interviewVendor) === -1 || !vendorInfo || yoke.sessionTools.capability(interviewVendor).kind === "none") engineError = "The selected interview engine is no longer available for assisted scheduling.";
        else if (!knownModels.length) engineError = "Interview models are still loading or unavailable for this engine.";
        else if (requestedModel && !catalogHasModel(knownModels, requestedModel)) engineError = "The selected interview model is no longer available.";
        else if (!catalogSupportsEffort(knownModels, requestedModel, requestedEffort, vendorInfo.effortLevels)) engineError = "The selected interview effort is unavailable for this engine.";
        if (engineError) {
          if (msg.requestId) sendTo(ws, { type: "new_session_result", requestId: String(msg.requestId).substring(0, 200), ok: false, error: engineError });
          return true;
        }
        var preferenceUserId = ws._clayUser ? ws._clayUser.id : "default";
        var savedPreference;
        try { savedPreference = usersModule.setScheduledTaskInterviewEngine(preferenceUserId, { vendor: interviewVendor, model: requestedModel, effort: requestedEffort }); }
        catch (preferenceError) { savedPreference = { error: preferenceError.message || String(preferenceError) }; }
        if (!savedPreference || !savedPreference.ok) {
          if (msg.requestId) sendTo(ws, { type: "new_session_result", requestId: String(msg.requestId).substring(0, 200), ok: false, error: savedPreference && savedPreference.error || "The interview engine preference could not be saved." });
          return true;
        }
      }
      ...
      if (msg.scheduleInterview === true && msg.model) sessionOpts.model = msg.model;
      ...
      if (requestedMode === "tui") {
        ...
        sessionOpts.effort = msg.scheduleInterview === true && msg.effort
          ? msg.effort
          : yoke.clampEffort(newSessionVendor, (sm.currentEffortByVendor && sm.currentEffortByVendor[newSessionVendor]) || sm.currentEffort || "medium") || null;
