...
function attachModels(ctx) {
  ...
  function getVendorAvailability(ws) {
    if (sm.installedVendors === undefined && sdk && typeof sdk.refreshInstalledVendors === "function") {
      sdk.refreshInstalledVendors(getLinuxUserForWs(ws));
    }
  ...
  async function resolveConfiguredModel(ws, alias) {
    ...
    var availability = getVendorAvailability(ws);
    var candidates = [];
