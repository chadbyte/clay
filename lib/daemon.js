...
var relay = createServer({
  ...
  onCreateWorktree: function (parentSlug, branchName, baseBranch) {
    ...
    var wtTitle = (parent.title || parent.slug) + " (" + branchName + ")";
...
function registerWorktrees(p) {
  ...
  for (var wi = 0; wi < worktrees.length; wi++) {
    ...
    // Title shows dir name, with branch in brackets if different
    var wtTitle = (p.title || p.slug) + " (" + wt.dirName + ")";
    if (wt.branch && wt.branch !== wt.dirName) {
      wtTitle = (p.title || p.slug) + " (" + wt.dirName + " → " + wt.branch + ")";
    }
