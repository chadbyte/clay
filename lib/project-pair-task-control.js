...
function attachPairTaskControl(ctx) {
  ...
  function cancelAll(worker, autonomousRunId) {
    var queue = queueFor(worker);
    var cancelled = 0;
    for (var i = 0; i < queue.length; i++) {
      if (queue[i].status !== "queued" && queue[i].status !== "waiting_for_interrupt") continue;
      if (autonomousRunId && queue[i].autonomousRunId !== autonomousRunId) continue;
  ...
  function complete(worker, caller, token, observedStatus, response, error) {
    ...
    if (ctx.sm.sendAndRecord) {
      try { ctx.sm.sendAndRecord(worker, { type: "pair_task_completed", _internal: true,
        ...
        workerOriginId: worker.sessionOriginId || null, taskId: token.taskId, generation: token.generation,
        outcome: envelope }); } catch (evidenceError) { token._completionEvidenceError = evidenceError.message || String(evidenceError); }
    }
