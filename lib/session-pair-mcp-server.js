...
var evaluationSchema = z ? z.union([
  z.enum(EVALUATION_OUTCOMES),
  z.object({ outcome: z.enum(EVALUATION_OUTCOMES), note: z.string().optional() }),
]) : null;
...
function getToolDefs(handlers, options) {
  ...
  var defs = [
    {
      ...
      inputSchema: buildShape({
        ...
        workerId: { type: "number", description: "Optional exact Worker session id; required when multiple Workers are configured." },
      }, ["message"]),
    ...
    },
    {
      ...
      inputSchema: buildShape({ reason: { type: "string", description: "Why the Driver is interrupting this task." }, workerId: { type: "number", description: "Optional exact Worker session id; required when multiple Workers are configured." } }),
    ...
    },
    {
      ...
      inputSchema: buildShape({ workerId: { type: "number", description: "Optional exact Worker session id; required when multiple Workers are configured." } }),
  ...
  return defs.concat([
    ...
    {
      name: "partner_status",
      description: "Bounded capacity and continuity report for your Split Worker, for deciding reuse against replacement: context tokens used and the ratio of its window, current task and activity, vendor/model/effort, history size and idle time, whether it is safe to replace right now, and your own recorded results for earlier Worker generations. Returns no transcript. Prefer this over reading turns when you only need to decide. Omit workerId to discover all configured Workers and their current numeric handles; pass workerId for one Worker.",
      inputSchema: buildShape({ workerId: { type: "number", description: "Optional exact Worker session id. Omit to list all configured Workers and their current handles." } }),
    ...
    },
    {
      ...
      inputSchema: buildShape({
        ...
        evaluation: {
          ...
          description: "Optional assessment of the Worker being replaced. Use succeeded, partial, failed, or abandoned, or an object with outcome set to one of those values and an optional note. Omit when no assessment is needed.",
        },
    ...
    },
    {
      ...
      inputSchema: buildShape({
        outcome: { enum: EVALUATION_OUTCOMES, description: 'One of "succeeded", "partial", "failed", "abandoned".' },
