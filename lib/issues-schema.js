...
function normalizeGithub(value) {
  if (value == null) return null;
  var parse = require('./session-github').parseReference;
  if (value.url) {
    var ref = parse(value.url);
    if (ref.kind !== 'issue') throw new Error('Only GitHub issues can be connected.');
    return Object.assign(ref, { title: text(value.title || '', 'GitHub title', 200, false),
      state: choice(value.state, 'GitHub state', ['open', 'closed', 'unknown'], 'unknown'), checkedAt: Number(value.checkedAt) || Date.now() });
  }
  var repo = parse('https://github.com/' + value.repository + '/issues/1').repository;
  if (!/^[a-f0-9]{32}$/.test(value.attempt)) throw new Error('Invalid GitHub creation attempt.');
  return { repository: repo, attempt: value.attempt };
}
...
function snapshot(input, current) {
  ...
  var allowed = { title: true, summary: true, body: true, resolutionSummary: true, type: true, priority: true, status: true,
    context: true, linkedWorkSessions: true, resolutionHistory: true, verifiedCommit: true, closeReason: true, github: true };
  ...
  if (data.github !== undefined || base.github) result.github = normalizeGithub(data.github === undefined ? base.github : data.github);
