function stopOwnedPair(session, roles, turnControl, taskControl, workerPermission, reason) {
  ...
  taskControl.cancelAll(roles.worker);
  taskControl.markInterruption(roles.worker, "user", reason || "The human stopped this scheduled run.");
  workerPermission.cancelForSession(roles.worker, reason || "The human stopped this scheduled run.");
  var members = [roles.driver, roles.worker];
