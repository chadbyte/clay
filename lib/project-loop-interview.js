...
function activeInterviewId(session) {
  ...
  for (var i = 0; i < history.length; i++) {
    var entry = history[i];
    if (entry && entry.source === "loop_interview") current = entry.interviewId || null;
    if (entry && (entry.source === "loop_interview_cancel" || entry.source === "loop_interview_close") && entry.interviewId === current) current = null;
  }
  return current;
}
...
function attachLoopInterview(ctx) {
  ...
  function currentInterview(session) {
    return activeInterviewId(session);
  }
...
module.exports = { attachLoopInterview: attachLoopInterview, brief: brief, activeInterviewId: activeInterviewId };
