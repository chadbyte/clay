...
function attachScheduledTasks(ctx) {
  ...
  var records = createScheduledTaskRecords({ cwd: ctx.cwd, registry: registry, canUseSession: ctx.canUseSession, canAccessRecord: ctx.canAccessRecord, canClaimOwner: ctx.canClaimOwner, preflightRuntime: ctx.preflightRuntime, runtimeStatus: ctx.scheduledExecution ? function (record) { return ctx.scheduledExecution.runnable(record); } : null });
    ...
    return [
      ...
      },
      {
        ...
        inputSchema: { type: "object", properties: {
          ...
          date: { type: "string" }, time: { type: "string" }, scheduleRule: SCHEDULE_RULE_SCHEMA, execution: EXECUTION_SCHEMA, skipIfRunning: { type: "boolean" }, claimOwner: { type: "boolean" },
  ...
  async function engineState(ws, msg, session) {
    ...
    var targetVendor = runtimeInstalled.indexOf(requestedVendor) !== -1 ? requestedVendor : installed[0] || runtimeInstalled[0] || "";
    var catalogError = "";
  ...
  function canCreateInterview(ws, session) {
    return allowed(ws, session) && available(session);
  }

  return { getToolDefs: getToolDefs, getSystemPrompt: getSystemPrompt, handleMessage: handleMessage, activeInterviewId: activeInterviewId, canStartInterview: canStartInterview, canCreateInterview: canCreateInterview };
}
