...
function attachPairFactory(ctx) {
  ...
  function validateEffort(vendor, effort, exact) {
    ...
    if (requested && (!clamped || (exact && clamped !== requested))) {
  ...
  function preflightRuntime(spec, defaultVendor, exact) {
    var request = spec || {};
    ...
    if (exact && !(typeof request.model === "string" && request.model.trim())) throw new Error("an explicit model is required for vendor \"" + vendor + "\"");
    if (exact && Array.isArray(info.effortLevels) && info.effortLevels.length && !(typeof request.effort === "string" && request.effort.trim())) throw new Error("an explicit reasoning effort is required for vendor \"" + vendor + "\"");
    return {
      vendor: vendor,
      model: validateModel(vendor, request.model),
      effort: validateEffort(vendor, request.effort, exact),
  ...
  function createUnpairedWorkerForDriver(driver, args) {
    var runtime = preflightWorkerForDriver(driver, args);
    var worker = sm.createSessionRaw({
    ...
    try {
      worker.title = "Split Worker · " + ((yoke.getVendorInfo(runtime.vendor) || {}).displayName || runtime.vendor);
      if (runtime.explicitEffort) worker.loopSettings = { effort: runtime.effort };
      sessionProvenance.markWorker(driver, worker, sm.sessions);
      if (typeof sm.saveSessionFile === "function") {
        if (sm.saveSessionFile(driver) === false || sm.saveSessionFile(worker) === false) {
          throw new Error("Split Worker session persistence returned false");
        }
      }
      return worker;
    } catch (error) {
  ...
  function exactMutation(driver, worker, args, method) {
    var group = store.groupForMember(driver.localId);
    if (!group) throw new Error("split group not found");
    var role = splitRoles.normalizePair(group.pair, group.members);
    if (!role.ok || role.kind === "adhoc") throw new Error("split group roles changed");
    var created = createUnpairedWorkerForDriver(driver, args);
    var request = {
      groupId: group.id,
      driverId: driver.localId,
      driverOriginId: driver.sessionOriginId || null,
      expectedGroup: args.expectedGroup || group,
      expectedWorkerIds: Array.isArray(args.expectedWorkerIds) ? args.expectedWorkerIds.slice() : role.workerIds.slice(),
      newWorkerId: created.localId,
      newWorkerOriginId: created.sessionOriginId || null,
    };
    if (worker) {
      request.workerId = worker.localId;
      request.workerOriginId = worker.sessionOriginId || null;
      request.generation = worker._pairGeneration ||
        worker.sessionProvenance && worker.sessionProvenance.generation || null;
    }
    var result = store[method](driver.ownerId || null, request, ctx.multiWorkerFeature);
    if (!result.ok) {
      removeCreated(created);
      throw new Error(result.error);
    }
    sm.broadcastSessionList();
    sm.sendToSession(driver, { type: "pair_session_created", ok: true, group: result.group });
    return { driver: driver, worker: created, group: result.group };
  }

  function addWorkerForDriver(driver, args) {
    return exactMutation(driver, null, args, "addWorker");
  }

  function replaceWorkerForDriver(driver, worker, args) {
    return exactMutation(driver, worker, args, "replaceWorker");
  }

  function createOwnedPair(ownerId, args) {
    if (ctx.isMate) throw new Error("Pair sessions are only available in projects");
    var request = args || {};
    ...
    try {
      ...
      if (workerRuntime.explicitEffort) worker.loopSettings = { effort: workerRuntime.effort };
      sessionProvenance.markWorker(driver, worker, sm.sessions);
      if (typeof sm.saveSessionFile === "function") { sm.saveSessionFile(driver); sm.saveSessionFile(worker); }
      var result = store.createOwned(ownerId || null, { members: [driver.localId, worker.localId], pair: { driverId: driver.localId, workerId: worker.localId }, name: request.name || "Scheduled task" });
      if (!result.ok) throw new Error(result.error);
      sm.broadcastSessionList();
      return { driver: driver, worker: worker, group: result.group };
    } catch (error) {
      removeCreated(worker);
      removeCreated(driver);
      throw error;
    }
  }

  function removeOwnedPair(ownerId, created) {
    if (!created || !created.group || !store.dissolveOwned(ownerId || null, created.group.id)) return false;
    removeCreated(created.worker);
    removeCreated(created.driver);
    return true;
  }
  ...
  return {
    ...
    addWorkerForDriver: addWorkerForDriver,
    replaceWorkerForDriver: replaceWorkerForDriver,
    createOwnedPair: createOwnedPair,
    removeOwnedPair: removeOwnedPair,
