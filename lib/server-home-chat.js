...
function attachHomeChat(deps) {
  ...
  function isActorLive(ws, userId) {
    if (!users.isMultiUser()) return true;
    var claimed = ws && ws._clayUser && ws._clayUser.id;
    if (!claimed || claimed !== userId) return false;
    return typeof users.findUserById === "function" ? !!users.findUserById(claimed) : true;
  }

  function assertFreshFound(ws, found, userId) {
    if (!isActorLive(ws, userId)) throw new Error("Your account is no longer available.");
    var liveFound = findMateProject(userId, found.mate.id, true);
    if (!liveFound || liveFound.ctx !== found.ctx || !liveFound.mate) throw new Error("Mate is no longer available.");
    return liveFound;
  }
  ...
  var resolveIssueLink = require("./server-issue-links").attachIssueLinks({ projects: projects, service: deps.issuesService, sendMessage: sendMessage });
  var homeModels = attachHomeModels({ users: users, mates: mates, projects: projects, sendMessage: sendMessage, resolveDefaultAi: deps.resolveDefaultAi });
  ...
  async function ensureSessionModel(ws, found, userId, session, resolvedMateModel) {
    ...
    var resolved = resolvedMateModel || await homeModels.resolveMateModel(ws, found, userId, { useDefault: false });
  ...
  var homeDebate = attachHomeDebatePlanning({
    ...
    homeModels: homeModels, isActorLive: isActorLive,
  });
  var homeMateCreation = attachHomeMateCreation({ findMateProject: findMateProject, resolveHomeSession: resolveHomeSession, sessionReference: sessionReference, setupTap: setupTap, sendHistory: sendHistory, sendSessionList: sendSessionList, sendError: sendError, sendModelError: sendModelError, homeModels: homeModels, isActorLive: isActorLive });
  var homeClayEntry = attachHomeClayEntry({ findMateProject: findMateProject, ownsSession: ownsSession, sessionReference: sessionReference, setupSearchTap: setupSearchTap, sendHistory: sendHistory, sendSessionList: sendSessionList, sendError: sendError, sendModelError: sendModelError, homeModels: homeModels, isActorLive: isActorLive });
  ...
  async function openDefaultSession(ws, found, userId, msg) {
    ...
    if (!session) {
      ...
      assertFreshFound(ws, found, userId);
      session = sessionManager.createSession({ ownerId: userId, vendor: resolved.vendor, model: resolved.model, effort: resolved.effort || null }, null);
    } else {
  ...
  async function openNewSession(ws, found, userId, msg) {
    ...
    var resolved = await homeModels.resolveMateModel(ws, found, userId);
    assertFreshFound(ws, found, userId);
    var fresh = sessionManager.createSession({ ownerId: userId, vendor: resolved.vendor, model: resolved.model, effort: resolved.effort || null }, null);
  ...
  async function sendHomeMessage(ws, found, userId, msg, options) {
    ...
    if (!activeSession) {
      ...
      if (!activeSession) {
        activeSession = sessionManager.createSession({ ownerId: userId, vendor: resolved.vendor, model: resolved.model, effort: resolved.effort || null }, null);
      } else {
  ...
  function handleMessage(ws, msg) {
    ...
    if (msg.type === "issue_reference_resolve") { resolveIssueLink(ws, msg); return true; }
