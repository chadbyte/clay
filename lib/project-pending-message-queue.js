...
function createPendingMessageQueue(opts) {
  ...
  function edit(session, actor, id, revision, expectedSessionId, message) {
    ...
    if (!text.trim() && !(current && ((current.message.images && current.message.images.length) || (current.message.pastes && current.message.pastes.length)))) return result(false, "Message must contain text or an attachment", session, actor);
    if (current) {
      var validationError = validateMessage(Object.assign({}, current.message, { text: text }));
      if (validationError) return result(false, validationError, session, actor);
    }
    return mutate(session, actor, id, revision, expectedSessionId, function (items, index) { items[index].message = Object.assign({}, items[index].message, { text: text }); });
  }
