...
var serverSpeech = require("./server-speech");
...
var { attachDurableScheduler } = require("./durable-scheduler");
var { attachDurableSchedulerLifecycle } = require("./server-durable-scheduler");
...
function createServer(opts) {
  ...
  var durableScheduler = null;
  var durableSchedulerLifecycle = null;
  ...
  var issuesService = require("./issues-service").attachIssuesService({
    getProjects: function () { return projects; },
    isMultiUser: function () { return users.isMultiUser(); },
    canAccessProject: function (userId, status) {
      if (!status || !status.slug || !onGetProjectAccess) return false;
    ...
    osUsersEnabled: function () { return osUsers; },
    getOsUserInfoForActor: function (user) { return user && user.linuxUser ? require("./os-users").resolveOsUserInfo(user.linuxUser) : null; },
    findUserById: function (userId) { return users.findUserById(userId); },
  });
  ...
  var speech = serverSpeech.attachSpeech({ users: users, getMultiUserFromReq: getMultiUserFromReq,
    isRequestAuthed: isRequestAuthed, CONFIG_DIR: CONFIG_DIR });
  ...
  var appHandler = function (req, res) {
    ...
    if (speech.handleRequest(req, res, fullUrl)) return;
  ...
  var globalWs = serverGlobalWs.attachGlobalWs({
    ...
    onConnection: function (ws) {
      ...
      if (cursorSharingService) cursorSharingService.sendStateTo(ws);
    },
  });

  server.once("close", function () { speech.destroy(); });
  server.on("upgrade", function (req, socket, head) {
    if (speech.handleUpgrade(req, socket, head)) return;
  ...
  function addProject(cwd, slug, title, icon, projectOwnerId, worktreeMeta, extraOpts) {
    ...
    var ctx = createProjectContext({
      ...
      resolveDefaultAi: function (ws) {
        if (!defaultAiService) return Promise.resolve({ ready: false, error: "Default AI is not ready." });
        return defaultAiService.resolveForWs(ws);
      },
      ...
      sendCursorSharingState: function (ws) { if (cursorSharingService) cursorSharingService.sendStateTo(ws); },
  ...
  var schedulerNamespace = process.env.CLAY_DEV ? "dev" : "prod";
  durableScheduler = opts.durableScheduler || attachDurableScheduler({
    storageDir: path.join(CONFIG_DIR, "scheduler"),
    namespace: schedulerNamespace,
    globalConcurrency: opts.schedulerGlobalConcurrency || 4,
    perOwnerConcurrency: opts.schedulerPerOwnerConcurrency || 1,
  });
  durableSchedulerLifecycle = attachDurableSchedulerLifecycle({
    server: server,
    scheduler: durableScheduler,
    onError: function (err) {
      console.error("[server] Durable scheduler shutdown failed:", err && err.message ? err.message : err);
    },
  });
  ...
  function destroyAll() {
    speech.destroy();
    var shutdowns = [];
    shutdowns.push(durableSchedulerLifecycle.shutdown());
    projects.forEach(function (ctx, slug) {
