...
var pairTarget = require("./session-pair-target");
...
var proposalRunner = require("./project-worker-proposal-runner");
var creationProposalModule = require("./project-worker-creation-proposal");
...
function attachWorkerProposal(ctx) {
  ...
  var runner = proposalRunner.attachProposalRunner(Object.assign({}, ctx, { updateProposal: updateProposal }));
  var resumeDriver = runner.resumeDriver;
  var runWorker = runner.runWorker;
  var creationProposal = creationProposalModule.attachWorkerCreationProposal({
    sm: sm, splitStore: store, multiWorkerFeature: ctx.multiWorkerFeature,
    canOffer: canOffer, isEligible: isEligible, ensureModelCatalogs: ensureModelCatalogs,
    proposalOptions: proposalOptions, chooseRecommendation: chooseRecommendation,
    skipPermissionsEnabled: skipPermissionsEnabled, recommendationCanAutoAccept: recommendationCanAutoAccept,
    autoAcceptanceWs: autoAcceptanceWs, acceptProposal: acceptProposal, updateProposal: updateProposal,
    toolResult: toolResult, maxSummaryChars: MAX_SUMMARY_CHARS, maxPlanChars: MAX_PLAN_CHARS,
    maxTaskChars: MAX_TASK_CHARS, maxRationaleChars: MAX_RATIONALE_CHARS,
  });
  async function proposeReplacement(args, session) {
    ...
    var addressed;
    try { addressed = pairTarget.resolveGroupTarget(sm, group, session, args, {
      mutation: true, multiWorkerFeature: ctx.multiWorkerFeature,
    }); }
    catch (e) { return toolResult({ error: e.message || String(e) }); }
    var sourceWorkerId = addressed.workerId;
    var sourceWorkerOriginId = addressed.partner.sessionOriginId || null;
    var sourceGeneration = addressed.partner._pairGeneration ||
      addressed.partner.sessionProvenance && addressed.partner.sessionProvenance.generation || null;
    var sourceMembership = creationProposalModule.captureMembership(sm, session, group);
    if (!sourceMembership) return toolResult({ error: "The exact Driver/Split Worker identities are not durable enough to replace a Worker." });
    ...
    if (!isLiveSession(session) || !driverEligibility.isEligibleDriverSession(session, sm) ||
        ...
        liveGroup.pair.driverId !== session.localId || liveGroup.members.indexOf(sourceWorkerId) === -1 ||
        sm.sessions.get(sourceWorkerId) !== addressed.partner ||
        (addressed.partner.sessionOriginId || null) !== sourceWorkerOriginId ||
        (addressed.partner._pairGeneration || addressed.partner.sessionProvenance && addressed.partner.sessionProvenance.generation || null) !== sourceGeneration ||
        !creationProposalModule.matchesMembership(sm, session, liveGroup, sourceMembership)) {
      return toolResult({ error: "The Driver/Split Worker pair changed while replacement runtimes were loading." });
    }
    try { pairTarget.resolveGroupTarget(sm, liveGroup, session, args, {
      mutation: true, multiWorkerFeature: ctx.multiWorkerFeature,
    }); }
    catch (e) { return toolResult({ error: e.message || String(e) }); }
    ...
    var proposal = {
      ...
      sourceWorkerOriginId: sourceWorkerOriginId,
  ...
  async function acceptProposal(session, proposal, msg, ws, trustedAutoAcceptance) {
    ...
    if (!proposal.action && !isEligible(session)) {
      throw new Error("The Driver is no longer eligible to create this Split Worker");
    }
    var approvedGroup = null;
    if (proposal.action === "add") {
      var addition = creationProposal.validateAcceptance(session, proposal);
      if (!addition.ok) throw new Error("The Driver/Split Worker membership changed before the addition was approved");
      approvedGroup = addition.group;
    } else if (proposal.action === "replace") {
      approvedGroup = store.groupForMember(session.localId);
      var approvedRoles = splitRoles.normalizePair(approvedGroup && approvedGroup.pair, approvedGroup && approvedGroup.members);
      var approvedWorker = sm.sessions.get(proposal.sourceWorkerId);
      if (!driverEligibility.isEligibleDriverSession(session, sm) || !approvedGroup ||
          approvedGroup.id !== proposal.sourceGroupId || !approvedRoles.ok || approvedRoles.kind === "adhoc" ||
          approvedRoles.driverId !== session.localId || approvedRoles.workerIds.indexOf(proposal.sourceWorkerId) === -1 ||
          !approvedWorker || (approvedWorker.sessionOriginId || null) !== (proposal.sourceWorkerOriginId || null) ||
          (approvedWorker._pairGeneration || approvedWorker.sessionProvenance && approvedWorker.sessionProvenance.generation || null) !== proposal.sourceGeneration ||
          !creationProposalModule.matchesMembership(sm, session, approvedGroup, proposal.sourceMembership)) {
    ...
    try {
      if (proposal.action === "add") {
        var added = ctx.addWorkerForDriver(session, {
          workerVendor: vendor, workerModel: model, workerEffort: effort,
          expectedGroup: approvedGroup, expectedWorkerIds: proposal.sourceWorkerIds,
        });
        ctx.recordGenerationStart(session, added.worker);
        updateProposal(session, proposal, { status: "running", groupId: added.group.id, workerId: added.worker.localId });
        ...
        return { ok: true, status: "running", group: added.group, addition: added };
      }
      ...
      if (proposal.action === "replace") {
        ...
        var restoredRoles = splitRoles.normalizePair(restoredGroup && restoredGroup.pair, restoredGroup && restoredGroup.members);
        if (restoredGroup && restoredRoles.ok && restoredRoles.kind !== "adhoc" &&
            restoredRoles.driverId === session.localId && restoredRoles.workerIds.indexOf(proposal.sourceWorkerId) !== -1) {
          proposal.sourceGroupId = restoredGroup.id;
          proposal.sourceMembership = creationProposalModule.captureMembership(sm, session, restoredGroup);
        }
  ...
  async function respondToProposal(ws, msg) {
    ...
    if (!msg.accepted) {
      ...
      var declinedText = proposal.action === "replace"
        ...
        : proposal.action === "add"
          ? "[Additional Split Worker declined]\nContinue with the existing Split Worker and current plan."
  ...
  function getToolDefs(session, options) {
    ...
    var proposalTools = controlsOnly && !creationProposal.canMountProposal(session) ? [] : [{
      name: "propose_worker",
      ...
      handler: function (args) { return creationProposal.propose(args || {}, session); },
    }];
  ...
  function getSystemPrompt(session) {
    if (creationProposal.canMountProposal(session) && !isEligible(session)) {
      return "This Driver may configure one additional Split Worker through propose_worker. With two Workers, pass workerId to every targeted partner, queue, interrupt, replacement, close, permission, and targeted status operation. Call partner_status without workerId to discover current Worker handles. Give each Worker independent file ownership and bounded work. Do not assign simultaneous overlapping file edits unless the work is explicitly separated, such as by distinct paths or worktrees.";
    }
