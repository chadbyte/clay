...
function attachWorkerProposal(ctx) {
  ...
  function getSystemPrompt(session) {
    if (creationProposal.canMountProposal(session) && !isEligible(session)) {
      return "This Driver may configure one additional Split Worker through propose_worker. With two Workers, pass workerId to every targeted partner, queue, interrupt, replacement, close, permission, and status operation. Give each Worker independent file ownership and bounded work. Do not assign simultaneous overlapping file edits unless the work is explicitly separated, such as by distinct paths or worktrees.";
    }
