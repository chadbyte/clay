...
// Every Worker the hierarchy links to each Driver, keyed by Driver local id.
// This is the exact scope a Driver deletion cascades to, including hidden
// sessions and replaced generations.
function ownedWorkersByDriver(sessions) {
  var hierarchy = hierarchyFor(sessions);
  var result = {};
  for (var i = 0; i < sessions.length; i++) {
    var node = hierarchy[sessions[i].localId];
    if (!node || node.role !== "worker" || node.parentSessionId == null) continue;
    if (!result[node.parentSessionId]) result[node.parentSessionId] = [];
    result[node.parentSessionId].push(sessions[i]);
  }
  return result;
}
...
module.exports = {
  ...
  ownedWorkersByDriver: ownedWorkersByDriver,
  isWorker: isWorker,
