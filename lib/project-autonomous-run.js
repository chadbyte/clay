...
var attachWakeScheduler = require("./autonomous-run-scheduler").attachAutonomousRunScheduler;
var runRecord = require("./autonomous-run-record");
...
var project = runRecord.project;
var restoreStoredRun = runRecord.restore;
function attachAutonomousRun(ctx) {
  ...
  function clearTimer(session) {
    if (wakeScheduler && session && session.autonomousRun) {
      try { wakeScheduler.cancel(session, session.autonomousRun); }
      catch (error) { session.autonomousRun.schedulerError = cleanText(error.message || String(error), 1000); }
    }
  ...
  function scheduleLimit(session) {
    ...
    if (wakeScheduler) { wakeScheduler.schedule(session, run, "deadline", run.deadlineAt); return; }
    clearTimer(session);
  ...
  async function armNow(ws, session, msg, options) {
    ...
    try {
      var run = {
        ...
        continuationPending: false, wakeSequence: 0,
      };
  ...
  function consume(session, msg) {
    ...
    delete run.armToken;
    persist(session);
    try { scheduleLimit(session); } catch (error) { finish(session, "error", "The run deadline could not be stored: " + (error.message || error)); return false; }
    return true;
  }
  ...
  function dispatchContinuation(session, runId, epoch) {
    ...
    try {
      if (!sdk.pushMessage(session, text, null, { autonomousRunId: run.id })) {
        var started = sdk.startQuery(session, text, null, ctx.ensureProjectAccessForSession(session));
        var queryOwner = { session: session, runId: run.id, epoch: epoch, generation: Number(session._queryGeneration || 0) };
        session._autonomousQueryOwner = queryOwner;
        Promise.resolve(started).then(function () {
          if (session._autonomousQueryOwner === queryOwner) delete session._autonomousQueryOwner;
        }, function (error) {
          var current = sm.sessions.get(session.localId);
          var currentRun = session.autonomousRun;
          if (current !== session || session._autonomousQueryOwner !== queryOwner || !currentRun || currentRun.id !== queryOwner.runId ||
              Number(currentRun.cancellationEpoch || 0) !== queryOwner.epoch || Number(session._queryGeneration || 0) !== queryOwner.generation) return;
          delete session._autonomousQueryOwner;
  ...
  function scheduleRateLimitWake(session, run) {
    ...
    if (wakeScheduler) {
      wakeScheduler.cancel(session, run);
      wakeScheduler.schedule(session, run, "deadline", run.deadlineAt);
      wakeScheduler.schedule(session, run, "rate-limit", wakeAt);
      return;
    }
    clearTimer(session);
  ...
  function continueRun(session, run) {
    ...
    persist(session);
    if (wakeScheduler) {
      try { wakeScheduler.schedule(session, run, "continuation", Date.now()); }
      catch (error) { finish(session, "error", "The continuation could not be stored: " + (error.message || error)); }
    } else setTimeout(function () { dispatchContinuation(session, run.id, epoch); }, 0);
  }
    ...
    serialize(session, async function () {
      try {
        if (msg.type === "autonomous_run_arm") await armNow(ws, session, msg);
          ...
          run.waitingReason = "";
          persist(session);
          try { scheduleLimit(session); }
          catch (error) {
            await finishNow(session, "error", "The run deadline could not be stored: " + (error.message || error));
            throw error;
          }
          continueRun(session, run);
        }
  ...
  function recover() {
    sm.sessions.forEach(function (session) {
      ...
      if (wakeScheduler) {
        try { wakeScheduler.cancel(session, run); }
        catch (error) { run.schedulerError = cleanText(error.message || String(error), 1000); }
      }
      if (run.state === "armed") {
  ...
  function onRateLimit(session, resetsAt) {
    ...
    persist(session);
    try { scheduleRateLimitWake(session, run); }
    catch (error) { finish(session, "error", "The rate-limit wake could not be stored: " + (error.message || error)); }
  }

  if (ctx.durableScheduler) {
    wakeScheduler = attachWakeScheduler({
      scheduler: ctx.durableScheduler, projectId: ctx.projectSlug, sm: sm, isMultiUser: ctx.isMultiUser,
      authorize: ctx.authorizeSession,
      onWake: function (kind, session, run) {
        if (kind === "deadline") return finishOwned(session, run.id, "limit", "The wall-clock limit was reached.");
        if (kind === "rate-limit") {
          run.rateLimitUntil = null; run.waitingReason = ""; persist(session); scheduleLimit(session); continueRun(session, run); return true;
        }
        dispatchContinuation(session, run.id, run.cancellationEpoch); return true;
      },
    });
  } else if (ctx.allowInMemoryTimers !== true) throw new Error("Until complete requires the durable scheduler.");
  recover();
  if (wakeScheduler) wakeScheduler.start();
  return { consume: consume, finish: finish, startApprovedBrief: startApprovedBrief, getSystemPrompt: getSystemPrompt, getToolDefs: getToolDefs, handleMessage: handleMessage, onHumanMessage: onHumanMessage, onPartnerResult: onPartnerResult, onRateLimit: onRateLimit, onTurnDone: onTurnDone, onUserAnswer: onUserAnswer, project: project, stopForPermissionChange: stopForPermissionChange,
    shutdown: function () { if (wakeScheduler) wakeScheduler.shutdown(); } };
}
