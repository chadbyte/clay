...
function attachAutonomousRun(ctx) {
  ...
  async function armNow(ws, session, msg) {
    ...
    try {
      ...
      persist(session);
      sendTo(ws, { type: "autonomous_run_arm_result", ok: true, sessionId: session.localId, requestId: cleanText(msg.requestId, 200), runId: run.id, armToken: run.armToken, run: project(run) });
    } catch (error) {
    ...
    sm.sessions.forEach(function (session) {
      ...
      if (run.state === "armed") {
        serialize(session, function () {
          return fullAccess.restore(session, run.permissionSnapshot).then(function () { session.autonomousRun = null; persist(session); }, function (error) {
