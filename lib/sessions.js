...
function createSessionManager(opts) {
  ...
  function sessionFilePath(recordId) {
    return path.join(sessionsDir, recordId + ".jsonl");
  }

  // A Clay record must exist before a provider assigns its resumable id.
  // Existing records keep their filename; fresh provider-less sessions use
  // the already durable origin id and later store cliSessionId as metadata.
  function sessionRecordId(session) {
    if (!session) return null;
    return session._sessionRecordId || session.cliSessionId || session.sessionOriginId || null;
  }

  function existingSessionRecordId(session) {
    if (!session) return null;
    return session._sessionRecordId || session.cliSessionId || null;
  }

  function saveSessionFile(session) {
    try {
      var provenanceMeta = sessionProvenance.metadata(session);
      var recordId = sessionRecordId(session);
      ...
      if (session.githubLinks) metaObj.githubLinks = session.githubLinks;
      ...
      if (session.mcpPermissionModeOverrides) metaObj.mcpPermissionModeOverrides = session.mcpPermissionModeOverrides;
      if (session.permissionModeBeforeFullAccess) metaObj.permissionModeBeforeFullAccess = session.permissionModeBeforeFullAccess;
      if (session.autonomousRun) metaObj.autonomousRun = session.autonomousRun;
      if (session.loopInterviewBrief) metaObj.loopInterviewBrief = session.loopInterviewBrief;
      ...
      var sfPath = sessionFilePath(recordId);
      ...
      session._sessionRecordId = recordId;
      return true;
    } catch(e) {
  ...
  function appendToSessionFile(session, obj) {
    var recordId = existingSessionRecordId(session);
    if (!recordId) return true;
    session.lastActivity = Date.now();
    try {
      var afPath = sessionFilePath(recordId);
  ...
  // Records that could not be read or parsed may still be live sessions.
  // Split-group recovery reads this so it never treats them as deleted.
  var sessionLoadUncertain = false;

  function loadSessions() {
    var files;
    try { files = fs.readdirSync(sessionsDir); } catch (e) {
      if (!e || e.code !== "ENOENT") sessionLoadUncertain = true;
      return;
    }
    ...
    for (var i = 0; i < files.length; i++) {
      ...
      var content;
      try { content = fs.readFileSync(path.join(sessionsDir, files[i]), "utf8"); } catch { sessionLoadUncertain = true; continue; }
      var lines = content.trim().split("\n");
      if (lines.length === 0) continue;

      var meta;
      try { meta = JSON.parse(lines[0]); } catch { sessionLoadUncertain = true; continue; }
      var pendingOrigin = meta && typeof meta.sessionOriginId === "string" && meta.sessionOriginId.trim() && meta.sessionOriginId.length <= 128;
      if (!meta || meta.type !== "meta" || (!meta.cliSessionId && !pendingOrigin)) { sessionLoadUncertain = true; continue; }
      ...
      loaded.push({ meta: meta, history: history, mtime: fileMtime, recordId: files[i].slice(0, -".jsonl".length) });
    }
    ...
    for (var i = 0; i < loaded.length; i++) {
      ...
      var session = {
        ...
        _sessionRecordId: loaded[i].recordId,
        blocks: {},
        sentToolResults: {},
        pendingPermissions: {},
        pendingAskUser: {},
        ...
        githubLinks: Array.isArray(m.githubLinks) ? m.githubLinks.slice(0, 8) : [],
      ...
      // Runtime-effective state is observed from the live SDK only. A daemon
      // restore has no active runtime to confirm the old value.
      session.effectivePermissionMode = null;
      session.mcpPermissionModeOverrides = m.mcpPermissionModeOverrides && typeof m.mcpPermissionModeOverrides === "object" ? m.mcpPermissionModeOverrides : {};
      session.permissionModeBeforeFullAccess = m.permissionModeBeforeFullAccess || null;
      session.autonomousRun = restoreStoredRun(m.autonomousRun);
      session.loopInterviewBrief = m.loopInterviewBrief && typeof m.loopInterviewBrief === "object" ? m.loopInterviewBrief : null;
    ...
    sessions.forEach(function (session) {
      ...
      try {
        ...
        fs.utimesSync(sessionFilePath(sessionRecordId(session)), preservedTime, preservedTime);
      } catch (e) {}
  ...
  function mapSessionForClient(s, clientActiveId, wsUnread) {
    ...
    return {
      ...
      effectivePermissionMode: s.effectivePermissionMode || null,
      permissionCapabilities: require("./session-permission-mode").capabilities(s, defaultVendor),
      mcpPermissionModeOverrides: s.mcpPermissionModeOverrides || {},
      autonomousRun: s.autonomousRun ? {
        id: s.autonomousRun.id,
        state: s.autonomousRun.state,
        continuationCount: s.autonomousRun.continuationCount || 0,
        maxContinuations: s.autonomousRun.maxContinuations,
        deadlineAt: s.autonomousRun.deadlineAt || null,
      } : null,
    };
  }

  // Counted over every session, including hidden ones, so delete
  // confirmations can state the full cascade. In multi-user mode only the
  // Driver's own owner receives it: another viewer could otherwise infer
  // hidden or private Workers, and a missing count means "unknown" to the
  // client, never zero. viewerId must come from the authenticated socket.
  function ownedWorkerCountFor(session, node, owned, viewerId) {
    if (node.role !== "driver") return 0;
    if (users.isMultiUser() && (!viewerId || (session.ownerId || null) !== viewerId)) return undefined;
    return owned[session.localId] ? owned[session.localId].length : 0;
  }

  function mapSessionsForClient(source, clientActiveId, wsUnread, viewerId) {
    var hierarchy = sessionProvenance.hierarchyFor(source);
    var owned = sessionProvenance.ownedWorkersByDriver([...sessions.values()]);
    return source.map(function (session) {
      ...
      var ownedCount = ownedWorkerCountFor(session, node, owned, viewerId);
      if (ownedCount !== undefined) mapped.ownedWorkerCount = ownedCount;
      ...
      sendEach(function (ws, filterFn) {
        ...
        if (ws.readyState === 1) {
          ws.send(JSON.stringify({
            type: "session_list",
            sessions: mapSessionsForClient(filtered, clientActiveId, wsUnread, ws._clayUser ? ws._clayUser.id : null),
          }));
        }
      });
    } else {
  ...
  function replayHistory(session, fromIndex, targetWs, transform) {
    ...
    for (var vi = 0; vi < sessionViewedListeners.length; vi++) {
      try { sessionViewedListeners[vi](session, targetWs, _send); } catch (e) {}
    }
  }
  ...
  function switchSession(localId, targetWs, transform) {
    ...
    if (!session) return;
    if (typeof onSessionHydrate === "function") onSessionHydrate(session);
  ...
  function deleteSession(localId, targetWs) {
    ...
    if (session.cliSessionId) rememberDeletedCliSession(session.cliSessionId);
    var recordId = existingSessionRecordId(session);
    if (recordId) try { fs.unlinkSync(sessionFilePath(recordId)); } catch(e) {}

    sessions.delete(localId);
    try { onSessionDeleted(localId, session); } catch (e) {
      console.error("[session] Session deletion hook failed:", e.message || e);
    }
    for (var di = 0; di < sessionDeletedListeners.length; di++) {
      try { sessionDeletedListeners[di](localId, session); } catch (e) {
        console.error("[session] Session deletion listener failed:", e.message || e);
      }
    }
  ...
  function sendAndRecordDurably(session, obj, clientObj) {
    var recordId = existingSessionRecordId(session);
    if (!recordId || !fs.existsSync(sessionFilePath(recordId))) return false;
    var saved;
  ...
  function hasDurableSessionRecord(session, predicate) {
    var recordId = existingSessionRecordId(session);
    if (!recordId || typeof predicate !== "function") return false;
    try {
      var lines = fs.readFileSync(sessionFilePath(recordId), "utf8").split("\n");
  ...
  return {
    setSessionHydrator: function (fn) { onSessionHydrate = typeof fn === "function" ? fn : null; },
    ...
    sessionLoadUncertain: function () { return sessionLoadUncertain; },
    setOnSessionDeleted: function (fn) { onSessionDeleted = fn || function () {}; },
    addOnSessionDeleted: function (fn) { if (typeof fn === "function") sessionDeletedListeners.push(fn); },
