  ...
  function handleFilesystemMessage(ws, msg) {
    ...
    if (/^fs_/.test(msg.type) && msg.type !== "fs_unwatch") {
      if (!access.canUseFiles(ws)) {
        sendTo(ws, { type: msg.type + "_result", path: msg.path, requestId: msg.requestId, projectSlug: msg.projectSlug, sessionId: msg.sessionId, accountId: msg.accountId, error: "File browser access is not permitted" });
        return true;
      }
      try { access.osIdentity(ws); } catch (e) {
        sendTo(ws, { type: msg.type + "_result", path: msg.path, requestId: msg.requestId, projectSlug: msg.projectSlug, sessionId: msg.sessionId, accountId: msg.accountId, error: e.message });
        return true;
      }
    }
    ...
    if (msg.type === "fs_read") {
      ...
      try {
        ...
        sendTo(ws, { type: "fs_read_result", path: msg.path, requestId: msg.requestId, projectSlug: msg.projectSlug, sessionId: msg.sessionId, accountId: msg.accountId, error: e.message });
      }
      return true;
    }
