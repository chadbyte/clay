...
function attachIssuesService(ctx) {
  ...
  var stores = new Map();
  var githubLocks = new Map();
  var githubAssessments = new Map();
  ...
  function bind(source, isSession) {
    ...
    var github = require('./issues-github').attachIssueGithub({
      locks: githubLocks, assessments: githubAssessments, run: context.runGithub, analyze: source.analyzeGithub || context.analyzeGithub,
      begin: function () {
        if (isSession) throw new Error('GitHub filing requires an explicit action in the Issues panel.');
        return operation(current(), true);
      },
      read: function (before, ref) {
        var state = recheck(before, true);
        return visibleIssue(openStore(state.status), ref, state);
      },
      save: function (before, ref, data, revision) {
        var state = recheck(before, true);
        var target = openStore(state.status);
        visibleIssue(target, ref, state);
        return target.update(ref, data, revision, before.actor);
      },
    });
    return {
      searchGithubIssue: github.search, linkGithubIssue: github.link, createGithubIssue: github.create,
