...
function runGh(cwd, args, identity, input) {
  return new Promise(function (resolve, reject) {
    ...
    var child = execFile(wrapped.command, wrapped.args, wrapped.options, function (error, stdout) {
      if (error) {
        var failure = new Error("GitHub is unavailable. Check gh authentication and repository access.");
        try { var response = JSON.parse(stdout); if (Number.isInteger(response.status)) failure.status = response.status; } catch (ignored) { /* CLI error without an API response. */ }
        return reject(failure);
      }
      try { resolve(JSON.parse(stdout)); } catch (e) { reject(new Error("GitHub returned an invalid response.")); }
    });
    child.stdin.on('error', function () { /* Process failure is handled above. */ });
...
module.exports = { parseReference: parseReference, readReference: readReference, runGh: runGh };
