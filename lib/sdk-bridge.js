...
function createSDKBridge(opts) {
  ...
  var isDriverOperatedSession = opts.isDriverOperatedSession || function () { return false; };
  ...
  function getAcpMcpServers(session) {
    ...
    var bridgeArgs = [
      ...
      "--query-generation", String(Number(session._sdkQueryGeneration || 0)),
      "--session-only",
    ];
  ...
  function getCodexSessionMcpServer(session, queryGeneration) {
    if (!session || !Number.isInteger(queryGeneration)) return null;
    var bridgeArgs = [
      path.join(__dirname, "yoke", "mcp-bridge-server.js"),
      "--port", String(clayPort),
      "--slug", slug,
      "--session", String(session.localId),
      "--query-generation", String(queryGeneration),
      "--session-only",
    ];
    if (clayTls) bridgeArgs.push("--tls");
    var bridgeEnv = {};
  ...
  var skills = attachSkillDiscovery({ cwd: cwd, getOptions: function () { return { env: getRuntimeEnv() }; } });
  ...
  function getSessionSkillOptions(session) {
    var runtimeEnv = getRuntimeEnv(session || {});
    if (!osUsers) return { env: runtimeEnv };
    var linuxUser = getSessionLinuxUser(session);
    if (!linuxUser) return { disable: true };
    try {
      var info = require("./os-users").resolveOsUserInfo(linuxUser);
      return Object.assign({}, skillDiscovery.optionsForOsUser(info), { env: runtimeEnv });
    } catch (e) {
      return { disable: true };
    }
  }
  ...
  var msgProcessor = attachMessageProcessor({
    ...
    getSessionSkillOptions: getSessionSkillOptions,
  ...
  function checkToolWhitelist(toolName, input) {
    ...
    // Session search is read-only; its handler enforces project and session access.
    if (toolName === "search_other_driver_sessions"
        || toolName === "mcp__clay-handoff__search_other_driver_sessions") {
      return { behavior: "allow", updatedInput: input };
    }

    // Local work associations verify GitHub data but never mutate GitHub.
    if (["link_session_github", "get_session_github", "unlink_session_github"].some(function (name) {
    ...
    if (toolName.indexOf("mcp__clay-mate-creation__") === 0) {
      return { behavior: "allow", updatedInput: input };
    }
    // Structured user input only presents a question and waits for the user's
    // answer. Yoke may expose the same query-bound capability as a native
    // dynamic tool, its MCP fallback, or the legacy Clay MCP name.
    if (toolName === "ask_user_questions"
        || toolName === "mcp__yoke-user-input__ask_user_questions"
        || toolName === "mcp__clay-ask-user__ask_user_questions") {
      return { behavior: "allow", updatedInput: input };
    }
    if (toolName === "begin_scheduled_task_interview"
        || toolName === "propose_scheduled_task"
        || toolName === "mcp__clay-scheduled-tasks__begin_scheduled_task_interview"
        || toolName === "mcp__clay-scheduled-tasks__propose_scheduled_task") {
      return { behavior: "allow", updatedInput: input };
    }
    ...
    var safeCapsuleTools = {
      ...
      clay_tool_act: true,
      clay_tool_set: true,
      clay_tool_source: true,
    };
    if (toolName.indexOf("mcp__clay-tools__") === 0
        && safeCapsuleTools[toolName.substring(toolName.lastIndexOf("__") + 2)]) {
      return { behavior: "allow", updatedInput: input };
    }
    ...
    if (toolName.indexOf("mcp__clay-issues__") === 0 || toolName.indexOf("mcp__clay-logs__") === 0) {
      return { behavior: "allow", updatedInput: input };
    }
    ...
    // Workspace inspection and proposal posting are session-bound. Keep this
    // list exact so a future execution/mutation tool cannot inherit approval.
    var safeWorkspaceTools = {
      ...
      "mcp__clay-workspace__propose_project_assignment": true,
      "mcp__clay-workspace__propose_project_follow_up": true,
      "mcp__clay-workspace__get_assignment_status": true,
    };
    if (safeWorkspaceTools[toolName] || toolName.indexOf("mcp__clay-history__") === 0) {
      return { behavior: "allow", updatedInput: input };
    }
  ...
  function resolveSessionPermissionMode(session) {
    var mode = session.permissionMode || sm.currentPermissionMode;
    var router = getWorkerPermissionRouter();
    if (router && typeof router.inheritedPermissionMode === "function") {
      var inherited = router.inheritedPermissionMode(session);
      if (inherited) mode = inherited;
    }
    return mode;
  }

  function handlePreToolUsePolicy(session, toolName, input) {
    var debateDecision = homeDebateToolPolicy.initialToolDecision(session, toolName, input);
    if (debateDecision) return debateDecision;
    // The dynamic proposal tool is bound to this exact query session. It only
    // posts the inline proposal; starting the debate still requires the user.
    if (session.homeDebatePlanning === true && toolName === "propose_debate") {
      return { behavior: "allow", updatedInput: input };
    }
    // Mate creation proposals only present the exact-session approval card;
    // creating the Mate still requires the user's explicit action there.
    if (session.mateCreationMode === true && toolName === "propose_mate") {
      return { behavior: "allow", updatedInput: input };
    }
    ...
    if (session.loop && session.loop.active && session.loop.role !== "crafting") {
      if (toolName === "AskUserQuestion") {
        return { behavior: "deny", message: "Autonomous mode. Make your own decision." };
      }
      if (toolName === "EnterPlanMode") {
        return { behavior: "deny", message: "Do not enter plan mode. Execute directly." };
      }
      return { behavior: "allow", updatedInput: input };
    }
  ...
  function handleCanUseTool(session, toolName, input, opts) {
    var initialDecision = handlePreToolUsePolicy(session, toolName, input);
    if (initialDecision) return Promise.resolve(initialDecision);
    // Skip permissions removes only the remaining Clay prompt. The SDK's
    // deny/ask rules and Auto classifier have already run before this callback.
    var skipRemainingPermissions = resolveSessionPermissionMode(session) === "bypassPermissions" && toolName !== "AskUserQuestion";

    if (skipRemainingPermissions) return Promise.resolve({ behavior: "allow", updatedInput: input });

    // Auto-approve if tool was previously allowed for session
    if (session.allowedTools && session.allowedTools[toolName]) return Promise.resolve({ behavior: "allow", updatedInput: input });
  ...
  function ownsQueryRuntime(session, queryInstance, queryGeneration) {
    return session.queryInstance === queryInstance
      && Number(session._sdkQueryGeneration || 0) === Number(queryGeneration)
      && (!sm.sessions || typeof sm.sessions.get !== "function" || sm.sessions.get(session.localId) === session);
  }

  function recordRuntimeEffectivePermissionMode(session, queryInstance, queryGeneration, message) {
    if (!message || (message.yokeType !== "init" && message.yokeType !== "status") || !message.permissionMode) return false;
    if (!ownsQueryRuntime(session, queryInstance, queryGeneration)) return false;
    session.effectivePermissionMode = message.permissionMode;
    session.permissionModeFallbackReason = null;
    sm.saveSessionFile(session);
    sendToSession(session, { type: "config_state", sessionId: session.localId, vendor: session.vendor || "claude",
  ...
  async function processQueryStream(session, codexCatalogCandidate) {
    ...
    var myQueryGeneration = Number(session._sdkQueryGeneration || 0);
    ...
    try {
      for await (var msg of myQueryInstance) {
        ...
        if (msg && msg.type === "_worker_meta") {
          ...
          switch (msg.subtype) {
            ...
            case "permission_mode_changed":
              // Setter notifications are acknowledgements, not a new project
              // default and not proof of the runtime-effective mode. Only an
              // init/status event below may confirm effectivePermissionMode.
              sendToSession(session, { type: "config_state", model: session.model || defaultModelForVendor(session.vendor || "claude"), vendor: session.vendor || "claude", sessionId: session.localId, mode: session.permissionMode || sm.currentPermissionMode || "default", effectivePermissionMode: session.effectivePermissionMode || null, permissionCapabilities: session.permissionCapabilities || { auto: false, mcpOverride: false }, mcpPermissionModeOverrides: session.mcpPermissionModeOverrides || {}, effort: session.effort || sm.currentEffort || "medium", betas: sm.currentBetas || [] });
              break;
            case "worker_error":
        ...
        // change this field.
        recordRuntimeEffectivePermissionMode(session, myQueryInstance, myQueryGeneration, msg);
        if (codexCatalogCandidate && msg && msg.yokeType === "session_started" && msg.sessionId
            && session.queryInstance === myQueryInstance
            && Number(session._sdkQueryGeneration || 0) === codexCatalogCandidate.generation
            && session.cliSessionId === msg.sessionId
            && (!sm.sessions || typeof sm.sessions.get !== "function" || sm.sessions.get(session.localId) === session)) {
      ...
      if (ownsRuntime) {
        ...
        // Preserve only restored legacy MCP entries. Canonical Yoke requests
        // hold a live provider callback and must be cancelled with their query.
        var keepAskUser = {};
        for (var _tid in session.pendingAskUser) {
          ...
          else if (_pending && _pending.mode === "yoke" && _pending.respond && typeof _pending.respond.cancel === "function") {
            _pending.respond.cancel("Query closed");
          }
        }
      ...
      if (!session.isProcessing && typeof opts.onSessionAvailable === "function") opts.onSessionAvailable(session);
    }
  }
  ...
  async function startQuery(session, text, images, linuxUser, beforePush, onAccepted) {
    delete session._lastTurnInterrupted;
    session._queryGeneration = (session._queryGeneration || 0) + 1;
    session._queryStarting = true;
    try {
      return await startQueryInner(session, text, images, linuxUser, beforePush, onAccepted);
    } finally {
      ...
      if (!session.queryInstance && !session.isProcessing) session.taskStopRequested = false;
      if (!session.isProcessing && typeof opts.onSessionAvailable === "function") opts.onSessionAvailable(session);
    }
  }
  ...
  async function startQueryInner(session, text, images, linuxUser, beforePush, onAccepted) {
    ...
    if (linuxUser) {
      ...
      try {
        claudeOpts.originalHome = require("./os-users").resolveOsUserInfo(linuxUser).home;
      } catch (e) {
        claudeOpts.originalHome = null;
      }
    ...
    session._sdkQueryGeneration = Number(session._sdkQueryGeneration || 0) + 1;
    // Individual feature modules own their transport capability checks. Do not
    // suppress unrelated session tools merely because one feature cannot be
    ...
    var hasIssueCatalog = false;
    for (var issueToolIndex = 0; issueToolIndex < sessionToolDefs.length; issueToolIndex++) {
      if (sessionToolDefs[issueToolIndex].name === "create_issue") {
        hasIssueCatalog = true;
        break;
      }
    }
    var recoverCodexIssueTools = sessionAdapter.vendor === "codex"
      ...
      && hasIssueCatalog
      && Number(session.codexIssuesToolCatalogVersion || 0) < 1;
    function queryContextAvailable() {
      if (Number(session._sdkQueryGeneration || 0) !== queryGeneration) return false;
      if (sm && sm.sessions && typeof sm.sessions.get === "function" && sm.sessions.get(session.localId) !== session) return false;
      return canUseSessionTools(session) === true;
    }
    var hasScheduleTool = false;
    var hasScheduleInterviewTool = false;
    var scheduleInterviewActive = null;
    var scheduleQuestionLimit = null;
    for (var sessionToolIndex = 0; sessionToolIndex < sessionToolDefs.length; sessionToolIndex++) {
      if (yoke.sessionTools.SCHEDULE_TOOLS.indexOf(sessionToolDefs[sessionToolIndex].name) !== -1) hasScheduleTool = true;
      if (sessionToolDefs[sessionToolIndex].name === "begin_scheduled_task_interview") {
        hasScheduleInterviewTool = true;
        scheduleInterviewActive = sessionToolDefs[sessionToolIndex].scheduleInterviewActive || null;
        scheduleQuestionLimit = sessionToolDefs[sessionToolIndex].structuredQuestionLimit || null;
      }
    }
    var activeRun = session.autonomousRun && ["armed", "running", "reviewing", "waiting-worker", "waiting-user", "paused"].indexOf(session.autonomousRun.state) !== -1;
    var activeLoopInterview = !isDriverOperatedSession(session) && !session.loopInterviewHandoff && !activeRun && !!loopInterview.activeInterviewId(session);
    var structuredInputSession = hasScheduleInterviewTool || activeLoopInterview || (!isDriverOperatedSession(session) && session.ralphCraftingMode === true && session.loop && session.loop.role === "crafting");
    var requestedUserInputMode = session.debateSetupMode || session.mateCreationMode ? "fallback" : "auto";
    if (structuredInputSession) requestedUserInputMode = "fallback";
    var selectedUserInputMode = yoke.userInput.selectMode(sessionAdapter, requestedUserInputMode);
    function structuredInputContextAvailable() {
      if (!queryContextAvailable()) return false;
      // A schedule-triggered fallback shares the schedule feature's live
      // eligibility. Other structured-input flows keep their own eligibility
      // and are not coupled to Scheduled Tasks.
      if (!hasScheduleInterviewTool || !scheduleInterviewActive || scheduleInterviewActive() !== true) return true;
      var liveDefs = getSessionToolDefs(session) || [];
      for (var liveIndex = 0; liveIndex < liveDefs.length; liveIndex++) {
        if (liveDefs[liveIndex].name === "begin_scheduled_task_interview") return true;
      }
      return false;
    }
    function scheduleQuestionError(request) {
      var questions = request && request.questions;
      if (!scheduleQuestionLimit || scheduleQuestionLimit() !== 1 || !Array.isArray(questions) || questions.length === 1) return null;
      return "Ask exactly one question at a time, wait for the answer, then ask the next question.";
    }
    var sessionUserInputHandler = function(request, respond) {
      if (!structuredInputContextAvailable()) {
        return respond.cancel("This structured-input request belongs to an unavailable or older query.");
      }
      var questionError = scheduleQuestionError(request);
      if (questionError) return respond.cancel(questionError);
      if (request.presentation === "elicitation" && request.diagnostics && request.diagnostics.elicitation) {
        var elicitationController = new AbortController();
        if (respond && typeof respond.onSettle === "function") {
          respond.onSettle(function (result) {
            if (result.status !== "submitted") elicitationController.abort();
          });
        }
        return handleElicitation(session, request.diagnostics.elicitation, { signal: elicitationController.signal }).then(function(result) {
          if (result && result.action === "accept") return respond.submitContent(result.content || {});
          return respond.cancel("Elicitation rejected");
        });
      }
      if (typeof opts.onUserInputRequest !== "function") return respond.cancel("Structured user input is unavailable.");
      return opts.onUserInputRequest(session, request, respond);
    };
    var localMcpServers = getMcpServers(session);
    if (sessionAdapter.vendor === "claude" && hasScheduleTool && typeof sessionAdapter.createToolServer === "function") {
      var scheduledTaskToolDefs = [];
      for (var scheduledToolIndex = 0; scheduledToolIndex < sessionToolDefs.length; scheduledToolIndex++) {
        if (yoke.sessionTools.SCHEDULE_TOOLS.indexOf(sessionToolDefs[scheduledToolIndex].name) !== -1) scheduledTaskToolDefs.push(sessionToolDefs[scheduledToolIndex]);
      }
      if (scheduledTaskToolDefs.length) {
        var scheduledTaskServer = sessionAdapter.createToolServer({ name: "clay-scheduled-tasks", version: "1.0.0", tools: scheduledTaskToolDefs });
        if (scheduledTaskServer) {
          localMcpServers = Object.assign({}, localMcpServers || {});
          localMcpServers["clay-scheduled-tasks"] = scheduledTaskServer;
        }
      }
    }
    if (selectedUserInputMode === "fallback") {
      var userInputServer = yoke.userInput.createFallbackServer(sessionAdapter, sessionUserInputHandler, { force: true, maxQuestions: scheduleQuestionLimit });
      if (userInputServer) {
        localMcpServers = Object.assign({}, localMcpServers || {});
        localMcpServers[userInputServer.name || "yoke-user-input"] = userInputServer;
      }
    }
    var mergedMcpServers = mergeMcpServers(localMcpServers, getRemoteMcpServers) || undefined;
    if (selectedUserInputMode === "fallback") {
      sessionToolDefs = sessionToolDefs.concat(yoke.userInput.fallbackToolDefs(sessionUserInputHandler, { maxQuestions: scheduleQuestionLimit }));
    }
    queryToolSnapshots.set(session, {
      generation: queryGeneration,
      defs: sessionToolDefs.slice(),
    });
    function findSessionToolDef(toolName) {
      ...
      var snapshotDef = null;
      for (var snapshotIndex = 0; snapshotIndex < sessionToolDefs.length; snapshotIndex++) {
        if (sessionToolDefs[snapshotIndex].name === toolName) {
          snapshotDef = sessionToolDefs[snapshotIndex];
          break;
        }
      }
      if (snapshotDef && snapshotDef.queryBound === true) return queryContextAvailable() ? snapshotDef : null;
    ...
    var modelSwitchGeneration = session._queryGeneration;
    var queryOpts = {
      onModelSwitch: function(change) {
        if (session._queryGeneration !== modelSwitchGeneration || session.taskStopRequested) return;
        if (!change || !change.fromModel || !change.toModel || change.fromModel === change.toModel) return;
      ...
      sessionMcpServer: recoverCodexIssueTools ? getCodexSessionMcpServer(session, queryGeneration) : undefined,
      ...
      canUseTool: function(toolName, input, toolOpts) {
        var nativeQuestionError = toolName === "AskUserQuestion" ? scheduleQuestionError(input) : null;
        if (nativeQuestionError) return { behavior: "deny", message: nativeQuestionError };
      ...
      preToolUsePolicy: function(toolName, input) {
        var nativeQuestionError = toolName === "AskUserQuestion" ? scheduleQuestionError(input) : null;
        if (nativeQuestionError) return Promise.resolve({ behavior: "deny", message: nativeQuestionError });
        var permissionName = toolName;
        var permissionToolDef = findSessionToolDef(toolName);
        if (permissionToolDef && permissionToolDef.permissionName) permissionName = permissionToolDef.permissionName;
        return handlePreToolUsePolicy(session, permissionName, input);
      },
      onUserInputRequest: function(request, respond) {
        return sessionUserInputHandler(request, respond);
      },
      validateUserInputRequest: scheduleQuestionError,
      userInputMode: selectedUserInputMode,
    ...
    // Capabilities come from the installed query handle, never a model-name
    // table. `acceptEdits` and MCP overrides are Claude SDK semantics; a
    // vendor may expose the generic setter without implementing either.
    session.permissionCapabilities = require("./session-permission-mode").capabilities(session, sm.defaultVendor);
    session.permissionCapabilities.mcpOverride = sessionAdapter.vendor === "claude" && typeof handle.setMcpPermissionModeOverride === "function";
    session.effectivePermissionMode = null;
    sm.saveSessionFile(session);
    sendToSession(session, { type: "config_state", model: session.model || defaultModelForVendor(session.vendor || "claude"), vendor: session.vendor || "claude", sessionId: session.localId, mode: session.permissionMode || sm.currentPermissionMode || "default", effectivePermissionMode: null, permissionCapabilities: session.permissionCapabilities, mcpPermissionModeOverrides: session.mcpPermissionModeOverrides || {}, effort: session.effort || sm.currentEffort || "medium", betas: sm.currentBetas || [] });

    if (session.permissionMode === "auto" && session.permissionCapabilities.mcpOverride) {
      var overrideNames = Object.keys(session.mcpPermissionModeOverrides || {});
      for (var oi = 0; oi < overrideNames.length; oi++) {
        try { await handle.setMcpPermissionModeOverride(overrideNames[oi], session.mcpPermissionModeOverrides[overrideNames[oi]]); }
        catch (overrideError) {
          sendToSession(session, { type: "error", text: "Failed to restore the AI review policy for " + overrideNames[oi] + ": " + (overrideError.message || overrideError) });
        }
      }
    }

    if (typeof beforePush === "function" && beforePush() !== true) {
    ...
    var codexCatalogCandidate = null;
    if (sessionAdapter.vendor === "codex" && !resumedAtQueryStart) {
      var requiredCatalogTools = { begin_scheduled_task_interview: false, propose_scheduled_task: false, list_scheduled_tasks: false, read_scheduled_task: false, update_scheduled_task: false, report_scheduled_task_outcome: false, ask_user_questions: false };
      for (var catalogToolIndex = 0; catalogToolIndex < sessionToolDefs.length; catalogToolIndex++) {
        if (Object.prototype.hasOwnProperty.call(requiredCatalogTools, sessionToolDefs[catalogToolIndex].name)) requiredCatalogTools[sessionToolDefs[catalogToolIndex].name] = true;
      }
      var completeInterviewCatalog = requiredCatalogTools.begin_scheduled_task_interview && requiredCatalogTools.propose_scheduled_task && requiredCatalogTools.list_scheduled_tasks && requiredCatalogTools.read_scheduled_task && requiredCatalogTools.update_scheduled_task && requiredCatalogTools.ask_user_questions;
      if (completeInterviewCatalog || requiredCatalogTools.report_scheduled_task_outcome) {
        codexCatalogCandidate = { generation: queryGeneration, version: yoke.sessionTools.CODEX_CATALOG_VERSION };
      }
      if (hasIssueCatalog) {
    ...
    session.streamPromise = processQueryStream(session, codexCatalogCandidate).catch(function(err) {
    });
    return true;
  }
  ...
  function pushMessage(session, text, images, deliveryMeta) {
    session.lastActivityAt = Date.now();
    ...
    if (session._queryStarting) {
      ...
      session.pendingPush.push(Object.assign({ text: text, images: images || null }, deliveryMeta || {}));
      return true;
    }
    return false;
  }
  ...
  function refreshEnvironmentRuntime() {
    if (sm && sm.sessions) {
      sm.sessions.forEach(function (session) { refreshSessionRuntime(session); });
    }
    var vendorNames = Object.keys(adapters);
    for (var i = 0; i < vendorNames.length; i++) {
      var runtime = adapters[vendorNames[i]];
      if (!runtime || typeof runtime.shutdownIfIdle !== "function") continue;
      Promise.resolve(runtime.shutdownIfIdle(0)).catch(function () {});
    }
    return true;
  }
  ...
  function refreshInstalledVendors(linuxUser) {
    sm.installedVendors = detectInstalledVendors(linuxUser);
    sm.availableVendors = getAvailableVendors(linuxUser);
    return sm.installedVendors.slice();
  }
  ...
  async function warmup(linuxUser) {
    ...
    refreshInstalledVendors(linuxUser);
    ...
    if (adapter) {
      try {
        ...
        // no authenticated session target, so never publish its personal
        // inventory through the project-wide session manager or broadcast.
  ...
  async function setMcpPermissionModeOverride(session, serverName, mode) {
    if (!session || !session.queryInstance || typeof session.queryInstance.setMcpPermissionModeOverride !== "function") return { ok: false, error: "MCP permission controls are unavailable for this session." };
    try {
      // Route through QueryHandle (works for both in-process and worker paths)
  ...
  return {
    ...
    handlePreToolUsePolicy: handlePreToolUsePolicy,
    ownsQueryRuntime: ownsQueryRuntime,
    recordRuntimeEffectivePermissionMode: recordRuntimeEffectivePermissionMode,
    ...
    warmup: warmup,
    refreshInstalledVendors: refreshInstalledVendors,
    stopTask: stopTask,
    ...
    getQueryToolDefs: function (session, queryGeneration) {
      var snapshot = session && queryToolSnapshots.get(session);
      if (!snapshot || snapshot.generation !== Number(queryGeneration) || Number(session._sdkQueryGeneration || 0) !== snapshot.generation) return [];
      return snapshot.defs.slice();
    },
  };
}
