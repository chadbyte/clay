...
var createProjection = require("./workspace-query-projection").createProjection;
...
function canonicalTurns(history, maxChars) {
  var turnLimit = maxChars == null ? MAX_TURN_CHARS : maxChars;
  var turns = [];
  var assistant = "";
  function flushAssistant() {
    var text = cleanText(assistant, turnLimit);
  ...
  for (var i = 0; i < (history || []).length; i++) {
    ...
    if (event.type === "delegated_work" || event.type === "delegated_follow_up") {
      ...
      flushAssistant();
      var userText = cleanText(event.text, turnLimit);
      if (userText) turns.push({ role: "user", text: userText });
    } else if (event.type === "delta" && typeof event.text === "string") {
      assistant += event.text;
      if (assistant.length > turnLimit * 2) assistant = assistant.substring(0, turnLimit * 2);
...
function attachWorkspaceQueryService(ctx) {
  ...
  function searchHistory(principal, projectSlug, args) {
    ...
    var projects = visible.map(function (project) {
      return projection.sanitizedProject(project, Infinity);
    });
    var max = decodeCursor(args && args.cursor) + parseLimit(args && args.limit, 20) + 20;
  ...
  function memorySessions(principal, excludeSource) {
    ...
    for (var p = 0; p < projects.length; p++) {
      ...
      var sanitized = projection.sanitizedProject(projects[p], MAX_TURN_CHARS);
