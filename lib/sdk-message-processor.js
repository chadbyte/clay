...
var turnQueue = require("./sdk-turn-queue");
var contextUsage = require("./context-usage");
...
function attachMessageProcessor(ctx) {
  ...
  var shouldSuppressResponseNotification = ctx.shouldSuppressResponseNotification || function () { return false; };
  ...
  var getSessionSkillOptions = ctx.getSessionSkillOptions || function () { return {}; };
  ...
  function processSDKMessage(session, parsed) {
    ...
    if (parsed.yokeType === "init") {
      ...
      var fsSkills = discoverSkillDirs(getSessionSkillOptions(session));
      session.skillNames = Array.from(mergeSkills(parsed.skills, fsSkills));
      if (parsed.slashCommands) {
        // Union: SDK slash_commands + merged skills (deduplicated)
        session.vendorSlashCommands = parsed.slashCommands.filter(function (name) {
          return session.skillNames.indexOf(name) === -1;
        });
        var seen = new Set();
        var combined = [];
        var all = session.vendorSlashCommands.concat(session.skillNames);
        ...
        session.slashCommandsByVendor[session.vendor || (adapter && adapter.vendor) || "claude"] = combined;
        sendToSession(session, { type: "slash_commands", commands: combined, vendor: session.vendor || (adapter && adapter.vendor) || "claude" });
      }
      if (parsed.model) {
    ...
    if (parsed.yokeType === "turn_start") {
      ...
      // Restored legacy MCP entries survive across turns. Canonical Yoke
      // requests own a live provider callback and are cancelled on turn end.
      ...
      for (var lai = 0; lai < leftoverAskIds.length; lai++) {
        ...
        if (lentry && lentry.mode === "yoke" && lentry.respond && typeof lentry.respond.cancel === "function") {
          lentry.respond.cancel("Turn ended");
        }
      ...
      session._queuedTurnCount = turnQueue.pendingSends(session, parsed);
      ...
      if (session.queryInstance && typeof session.queryInstance.getContextUsage === "function") {
        var contextQuery = session.queryInstance;
        var contextTurn = session.turnCount;
        contextUsage.readUsage(session, "summary").then(function(ctxUsage) {
          if (session.queryInstance !== contextQuery || session.turnCount !== contextTurn) return;
          // null means the shared reader suppressed a superseded response;
          // only an adapter's explicit unavailable object may clear the UI.
          if (!ctxUsage) return;
      ...
      sendAndRecord(session, {
        ...
        userMessageIds: parsed.userMessageIds,
        resultIndex: parsed.resultIndex,
        resumeReason: parsed.resumeReason,
        localCommand: parsed.localCommand,
      });
      if (parsed.fastModeState) {
        sendAndRecord(session, { type: "fast_mode_state", state: parsed.fastModeState });
      }
      ...
      var _suppressResponseNotification = false;
      try { _suppressResponseNotification = shouldSuppressResponseNotification(session) === true; } catch (e) {}

      if (pushModule && !session._delegatedBy && !_suppressResponseNotification) {
        pushModule.sendPush({
          type: "done",
          slug: slug,
          title: _doneTitle,
      ...
      if (_nm && !session.loop && !session._delegatedBy && !_suppressResponseNotification) {
      ...
      if (session.turnCount >= sessionTitlePolicy.generationTurn(isMate)
          && (!session.titleAutoGenerated || sessionTitlePolicy.isFailedGeneratedTitle(session))
      ...
      // Persist the scoped reset snapshot so reconnect/history restore can reproject it.
      ...
      if (info.status === "allowed_warning" || info.status === "rejected") {
        ...
        if (info.status === "rejected" && info.resetsAt) {
          ...
          if (typeof opts.onRateLimitRejected === "function") {
            opts.onRateLimitRejected(session, info.isUsingOverage ? Date.now() + 5000 : session.rateLimitResetsAt);
          }
      ...
      var cmdSeen = new Set();
      var cmdCombined = [];
      session.vendorSlashCommands = (parsed.commandNames || []).filter(function (name) {
        return (session.skillNames || []).indexOf(name) === -1;
      });
      var cmdAll = session.vendorSlashCommands.concat(session.skillNames || []);
      ...
      session.slashCommands = cmdCombined;
      session.slashCommandsByVendor = session.slashCommandsByVendor || {};
      session.slashCommandsByVendor[session.vendor || (adapter && adapter.vendor) || "claude"] = cmdCombined;
      sendToSession(session, { type: "slash_commands", commands: cmdCombined, vendor: session.vendor || (adapter && adapter.vendor) || "claude" });
