...
function attachPairTurnControl(ctx) {
  ...
  function markHumanStop(session) {
    ...
    state.stoppedWorkerId = roles.worker ? roles.worker.localId : null;
    state.stoppedWorkerIds = roles.workers.map(function (worker) { return worker.localId; });
    return roles;
  }
