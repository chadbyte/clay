...
var deletionCleanup = require("./session-split-group-cleanup");
...
function createSplitGroupStore(opts) {
  ...
  var cleanup = deletionCleanup.attachDeletionCleanup({
    sessions: sessions, groups: function () { return groups; }, save: save,
    broadcast: function () { broadcast(); }, onPairChanged: function (group) { onPairChanged(group); },
    retryDelaysMs: opts.persistRetryDelaysMs, setTimer: opts.setTimer, clearTimer: opts.clearTimer, logger: opts.logger,
  });
  function save() {
  ...
  // Deletion cleanup is irreversible; see session-split-group-cleanup.js.
  function dissolveBySession(localId) { return cleanup.sessionDeleted(localId); }
  ...
  if (persistence.loadRewriteError()) cleanup.recordFailure("startup rewrite", persistence.loadRewriteError());
  return { create: create, createOwned: createOwned, dissolveOwned: dissolveOwned,
    ...
    persistenceStatus: cleanup.status, retryPersistence: cleanup.retryNow, shutdown: cleanup.shutdown,
...
function attachSplitGroups(ctx) {
  var store = createSplitGroupStore({
    ...
    sessionLoadUncertain: ctx.sm.sessionLoadUncertain,
  });
  ...
  return { handleMessage: handleMessage, sendConnectionState: sendState, store: store, shutdown: store.shutdown };
}
