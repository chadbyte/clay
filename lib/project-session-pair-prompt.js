...
function attachSessionPairPrompt(ctx) {
  function getSystemPrompt(session) {
    ...
    if (group && group.pair && group.pair.driverId === session.localId) {
      ...
      if (roles && roles.driver === session) {
        result += " Current configured Worker handles: " + roles.workers.map(function (worker) {
          return "workerId=" + worker.localId + " (" + (worker.title || "Worker") + ")";
        }).join(", ") + ". Call partner_status without workerId to rediscover current Workers; pass workerId for targeted operations.";
      }
