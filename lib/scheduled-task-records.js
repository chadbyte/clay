...
var PATCH_FIELDS = { name: true, instructions: true, cron: true, date: true, time: true, scheduleRule: true, execution: true, skipIfRunning: true, claimOwner: true };
...
function createScheduledTaskRecords(ctx) {
  ...
  function publicRecord(record) {
    ...
    return {
      ...
      needsOwner: record.needsOwner === true, setupRequired: record.setupRequired || null,
  ...
  function update(session, id, revision, patch) {
    ...
    var claimOwner = Object.prototype.hasOwnProperty.call(patch, "claimOwner");
    if (claimOwner && patch.claimOwner !== true) return { ok: false, error: "claimOwner must be true." };
    if (claimOwner && record.ownerId) return { ok: false, error: "This scheduled task already has an owner." };
    if (claimOwner && !session.ownerId) return { ok: false, error: "An authenticated owner is required to claim this scheduled task." };
    if (claimOwner && ctx.canClaimOwner && !ctx.canClaimOwner(session, record)) return { ok: false, error: "Only a project owner or administrator can claim this scheduled task." };
    if (claimOwner && patchKeys.length === 1) {
      if (!ctx.registry.update(record.id, { ownerId: session.ownerId, enabled: true })) return { ok: false, error: "The scheduled task owner could not be stored." };
    ...
    var checkedPatch = Object.assign({}, patch);
    delete checkedPatch.claimOwner;
    var merged = Object.assign({}, current, checkedPatch);
    ...
    try {
      ...
      if (claimOwner) { updateData.ownerId = session.ownerId; updateData.enabled = true; }
      if (updateData.cron) {
