var pairMcp = require("./session-pair-mcp-server"); var { attachPairFactory } = require("./session-pair-factory"), { attachPairLifecycle } = require("./project-pair-lifecycle"), { attachPairTurnControl } = require("./session-pair-turn-control");
var driverEligibility = require("./session-driver-eligibility"), { attachWorkerPermission } = require("./project-worker-permission"), { attachWorkerProposal } = require("./project-worker-proposal");
var attachSessionPairPrompt = require("./project-session-pair-prompt").attachSessionPairPrompt;
var attachPairCloseTransaction = require("./project-pair-close-transaction").attachPairCloseTransaction;
var { attachPairMessage } = require("./project-pair-message"), { attachPairTaskControl } = require("./project-pair-task-control"), stopAutonomous = require("./project-pair-autonomous-stop").stopAutonomousWork, stopOwned = require("./project-pair-owned-stop").stopOwnedPair;
...
var beginPairQuery = require("./session-pair-query-start").beginPairQuery, attachPairResultPipeline = require("./project-pair-result-pipeline").attachPairResultPipeline;
var pairTarget = require("./session-pair-target"), splitRoles = require("./session-split-group-roles"), stopAllWorkers = require("./project-pair-global-stop").stopAllWorkers;
function toolResult(value) { return Promise.resolve({ content: [{ type: "text", text: JSON.stringify(value) }] }); } function toolError(err) { return Promise.resolve({ content: [{ type: "text", text: "Error: " + (err.message || err) }], isError: true }); }
function attachSessionPair(ctx) {
  var sm = ctx.sm, store = ctx.splitStore, workerProposal, workerPermission, lifecycle, taskControl, closeTransaction;
  var turnControl = attachPairTurnControl({ sm: sm, splitStore: store }), resultCapture;
  function targetOptions(options) { return Object.assign({}, options || {}, { multiWorkerFeature: ctx.multiWorkerFeature }); }
  function groupAndPartner(caller, missingPairMessage) {
    ...
    return pairTarget.resolveGroupTarget(sm, group, caller, args, targetOptions(options));
  }
  function rolesFor(session) {
    return pairTarget.rolesFor(sm, store, session);
  }
  var pairMessage = attachPairMessage({
    sm: sm,
    getSdk: ctx.getSdk,
    resolvePair: function (caller, args) { return groupAndPartner(caller, null, args, { mutation: true }); },
    turnControl: turnControl,
  });
  taskControl = attachPairTaskControl({
    sm: sm, resolvePair: function (caller, args, options) { return groupAndPartner(caller, null, args, options); }, rolesFor: rolesFor, turnControl: turnControl,
  ...
  function resumeDriverWithMessage(caller, text, meta) {
    ...
    if (!sdk.pushMessage(caller, text)) {
      beginPairQuery({ sdk: sdk, sm: sm, linuxUser: ctx.getLinuxUserForSession(caller), onProcessingChanged: ctx.onProcessingChanged }, caller, text);
    }
  ...
  function requestDetach(worker) {
    ...
    if (token.outboxKey && ctx.resultOutbox && !ctx.resultOutbox.setRoute(token.outboxKey, "callback").ok) return false;
    token.wait = false; token.detachRequested = true;
    return true;
  }
  function handleTurnDone(partner) {
    var token = partner && partner._pairDelegation;
    if (!token) return false;
    if (token._capturePending || token._captureComplete) return false;
    if (!taskControl.matchesGeneration(partner, token)) return false;
    ...
    var liveRoles = splitRoles.resolveLiveRoles(sm, group);
    if (group.pair && (!liveRoles || liveRoles.driver !== caller || liveRoles.workers.indexOf(partner) === -1)) {
    ...
    var outcome = taskControl.complete(partner, caller, token, token.failure ? "failed" : (token.interrupted ? "interrupted" : "completed"), token.response, token.failure);
    return resultCapture.capture(caller, partner, token, outcome);
  }
  function waitForPartner(group, caller, partner, token, timeoutSeconds) {
    return new Promise(function (resolve, reject) {
      ...
      var timer = setInterval(function () {
        ...
        var exactTarget = splitRoles.matchesLiveTarget(sm, currentGroup, caller, partner);
        if (!currentGroup || currentGroup.id !== group.id || !exactTarget) {
          clearInterval(timer);
          finishDelegation(group, caller, partner, token);
        ...
        if (!partner.isProcessing && !partner._queryStarting) {
          clearInterval(timer);
          if (!taskControl.matchesGeneration(partner, token)) return reject(new Error("the delegated task no longer matches this Worker generation"));
          ...
          resultCapture.capture(caller, partner, token, outcome);
          resolve({
            status: token._capturePending ? "capture_pending" : (failure ? "error" : (outcome.status === "completed" ? "complete" : outcome.status)),
            taskId: token.taskId,
        ...
        if (Date.now() >= deadline || token.detachRequested) {
          clearInterval(timer);
          var routed = token.outboxKey && ctx.resultOutbox ? ctx.resultOutbox.setRoute(token.outboxKey, "callback") : { ok: true };
          if (!routed.ok) {
            monitorPartner(group, caller, partner, token);
            resolve({ status: "capture_pending", taskId: token.taskId, generation: token.generation, response: responseText(partner.history || [], token.startIndex), error: routed.error });
            return;
          }
          token.wait = false; token.detached = true;
          monitorPartner(group, caller, partner, token);
          resolve({ status: "running", taskId: token.taskId, generation: token.generation, response: responseText(partner.history || [], token.startIndex), hint: "The completed result will be pushed back automatically. Use read_partner only for a requested status, concrete failure or stall diagnosis, or capacity/queue decision; do not poll routinely." });
        }
      }, 500);
    });
  }
  function monitorPartner(group, caller, partner, token) {
    var timer = setInterval(function () {
      var currentGroup = store.groupForMember(caller.localId);
      var exactTarget = splitRoles.matchesLiveTarget(sm, currentGroup, caller, partner);
      if (!currentGroup || currentGroup.id !== group.id || !exactTarget || (!partner.isProcessing && !partner._queryStarting)) {
        clearInterval(timer);
        if (currentGroup && currentGroup.id === group.id && exactTarget) handleTurnDone(partner);
  ...
  async function sendToPartner(args, caller) {
    try {
      ...
      var resolved = groupAndPartner(caller,
        "send_to_partner requires an exact existing Driver/Split Worker pair; call propose_worker while unpaired", args, { mutation: true });
      ...
      var token = Object.assign(task, { from: caller.localId, groupId: resolved.group.id, startIndex: partner.history.length, wait: wait });
      var started = resultCapture.begin(caller, partner, token, message, ctx.projectSlug);
      if (!started.ok) {
        token._retainOnError = true; partner._pairDelegation = token; partner._delegatedBy = caller.localId;
        sm.sendAndRecord(caller, { type: "error", text: "The Split Worker task was not dispatched because its durable completion record could not be created: " + started.error });
        return toolError(new Error(started.error));
      }
      ...
      if (!wait) {
        ...
        return toolResult({ status: "running", partnerId: partner.localId, taskId: token.taskId, generation: token.generation, hint: "The completed result will be pushed back automatically. Use read_partner only for a requested status, concrete failure or stall diagnosis, or capacity/queue decision; do not poll routinely." });
      }
      if (!partner.isProcessing && !partner._queryStarting) {
        if (!taskControl.matchesGeneration(partner, token)) return toolError(new Error("the delegated task no longer matches this Worker generation"));
        ...
        var immediateCapture = resultCapture.capture(caller, partner, token, immediateOutcome);
        if (!immediateCapture && token._capturePending) return toolError(new Error("The completed Split Worker result could not be durably captured yet."));
        return toolResult({
      ...
      if (caller) {
        var group = store.groupForMember(caller.localId);
        if (group) {
          var failedRoles = splitRoles.resolveLiveRoles(sm, group);
          var failedTarget = failedRoles && splitRoles.resolveWorkerTarget(failedRoles.role, args && args.workerId);
          var partner = failedTarget && failedTarget.ok ? sm.sessions.get(failedTarget.workerId) : null;
          if (partner && partner._pairDelegation && partner._pairDelegation.from === caller.localId && !partner.isProcessing && !partner._pairDelegation._retainOnError) {
  ...
  function readPartner(args, caller) {
    try {
      var resolved = groupAndPartner(caller, null, args, { readOnly: true });
      ...
      payload.capacity = lifecycle.optionalStatus(caller, args);
  ...
  function interruptPartner(args, caller) {
    try {
      var resolved = groupAndPartner(caller, null, args, { mutation: true });
      var partner = resolved.partner;
      if (!partner.isProcessing && !partner._queryStarting) {
  ...
  function closeWorker(args, caller) {
    return closeTransaction.close(args, caller);
  }
  function closePartner(args, caller) {
    try {
      return toolResult(closeWorker(args, caller));
    } catch (e) {
      return toolError(e);
    }
  }
  function handleHumanStop(session) {
    ...
    return stopAllWorkers(session, roles, { resultOutbox: ctx.resultOutbox, taskControl: taskControl, workerPermission: workerPermission });
  }
  function stopOwnedPair(session, reason) { return stopOwned(session, rolesFor(session), turnControl, taskControl, workerPermission, reason); }
  function stopAutonomousWork(session) { return stopAutonomous(session, rolesFor(session), turnControl, taskControl, workerPermission); }
  function beginHumanTurn(session) {
    var resumed = turnControl.beginHumanTurn(session);
    var roles = resumed && rolesFor(session);
    if (roles) for (var i = 0; i < roles.workers.length; i++) taskControl.drain(roles.driver, roles.workers[i]);
    return resumed;
  }
  function stopBarrier(session) { return turnControl.blockedReason(session); }
  function getToolDefs(boundSession) {
    ...
    var boundRoles = group && group.pair ? splitRoles.resolveLiveRoles(sm, group) : null;
    if (boundRoles && boundRoles.workers.indexOf(boundSession) !== -1) return taskControl.workerToolDefs(boundSession);
    ...
    var tools = pairMcp.getToolDefs({
      send: function (args) {
        ...
        }, operationFingerprint(args, ["message", "wait", "timeoutSeconds", "taskId", "workerId"])).catch(toolError);
      },
      ...
      replace: function (args) {
        ...
        }, operationFingerprint(args, ["interrupt", "message", "workerVendor", "workerModel", "workerEffort", "recommendationRationale", "evaluation", "supersedeProposalId", "workerId"])).catch(toolError);
      },
  ...
  var factory = attachPairFactory(ctx), createPairRecord = factory.createPairRecord,
    ...
    addWorkerForDriver = factory.addWorkerForDriver, replaceWorkerForDriver = factory.replaceWorkerForDriver,
    preflightWorkerForDriver = factory.preflightWorkerForDriver, createOwnedPair = factory.createOwnedPair, removeOwnedPair = factory.removeOwnedPair,
    preflightRuntime = factory.preflightRuntime;
  lifecycle = attachPairLifecycle({
    sm: sm,
    splitStore: store,
    createWorkerForDriver: function (driver, args) { return createWorkerForDriver(driver, args); },
    replaceWorkerForDriver: function (driver, worker, args) { return replaceWorkerForDriver(driver, worker, args); },
  ...
  workerProposal = attachWorkerProposal({
    ...
    addWorkerForDriver: function (driver, args) { return addWorkerForDriver(driver, args); },
  ...
  var pairPrompt = attachSessionPairPrompt({ sm: sm, store: store, isMate: ctx.isMate, rolesFor: rolesFor,
    workerProposal: function () { return workerProposal; } });
  var resultPipeline = attachPairResultPipeline({ sm: sm, store: store, resultOutbox: ctx.resultOutbox, getSdk: ctx.getSdk, getLinuxUserForSession: ctx.getLinuxUserForSession, acceptanceTimeoutMs: ctx.acceptanceTimeoutMs, finishDelegation: finishDelegation, resumeDriverWithResult: resumeDriverWithResult, onPartnerResult: ctx.onPartnerResult, onStateChange: ctx.onResultStateChange, blockedReason: turnControl.blockedReason, drain: function (caller, partner) { return taskControl.drain(caller, partner); } });
  resultCapture = resultPipeline.capture; var resultDelivery = resultPipeline.delivery;
  closeTransaction = attachPairCloseTransaction({ store: store, multiWorkerFeature: ctx.multiWorkerFeature,
    resolvePair: function (caller, args) { return groupAndPartner(caller, null, args, { mutation: true, structural: true }); },
    taskControl: taskControl, resultCapture: resultCapture, workerPermission: workerPermission,
    responseText: responseText, errorSince: errorSince });
  return { beginHumanTurn: beginHumanTurn, getToolDefs: getToolDefs, handleHumanStop: handleHumanStop, stopOwnedPair: stopOwnedPair, stopAutonomousWork: stopAutonomousWork, stopBarrier: stopBarrier, handleMessage: handleMessage, handleTurnDone: handleTurnDone, getSystemPrompt: pairPrompt.getSystemPrompt,
    respondToWorkerProposal: workerProposal.respondToProposal, workerPermission: workerPermission, addWorkerForDriver: addWorkerForDriver, createOwnedPair: createOwnedPair, removeOwnedPair: removeOwnedPair, preflightRuntime: preflightRuntime,
    sendToPartner: sendToPartner, closeWorker: closeWorker, rolesFor: rolesFor, resultCapture: resultCapture, resultOutbox: ctx.resultOutbox, wakePartnerResult: function (caller, partner, token) { return resultDelivery.wake(caller, partner, token); }, getResultOutboxStatus: function () { return ctx.resultOutbox ? ctx.resultOutbox.status() : { records: [], persistenceError: null }; } };
