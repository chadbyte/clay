...
function createAntigravityAdapter(opts) {
  ...
  function capabilities() {
    return {
      ...
      queryBoundTools: { mode: "none", reason: "The current Clay adapter does not forward custom-tool configuration to agy." },
      skillSharing: true,
  ...
  var adapter = {
    ...
    init: async function(initOpts) {
      ...
      var skills = skillDiscovery.discoverSkills(cwd, { env: (initOpts && initOpts.env) || null }).map(function(skill) { return skill.name; });
      return {
