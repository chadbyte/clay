...
var claudeUserInput = require("../claude-user-input");
var claudeCorrelation = require("../claude-message-correlation");
var claudeContextUsage = require("../claude-context-usage");
var claudeModelSwitch = require("../claude-model-switch");
...
function applySystemPromptSnapshot(systemPrompt, enabled) {
  ...
  if (typeof systemPrompt === "string" || Array.isArray(systemPrompt)) {
    return { type: "custom", prompt: systemPrompt, snapshot: enabled };
  }
  if (typeof systemPrompt === "object") {
    return Object.assign({}, systemPrompt, { snapshot: enabled });
  }
  return systemPrompt;
}

function attachPreToolUsePolicy(options, policy) {
  if (typeof policy !== "function") return;
  options.hooks = options.hooks || {};
  options.hooks.PreToolUse = (options.hooks.PreToolUse || []).concat([{
    hooks: [async function(input, toolUseId, hookOpts) {
      input = input || {};
      var decision = await policy(input.tool_name, input.tool_input || {}, {
        toolUseID: toolUseId || input.tool_use_id,
        signal: hookOpts && hookOpts.signal,
      });
      ...
      return {
        hookSpecificOutput: {
          ...
          permissionDecisionReason: decision.message || "Clay session policy",
...
function getSharedSkillPlugins(homeDir, existingPlugins, cwd, env) {
  var plugins = Array.isArray(existingPlugins) ? existingPlugins.slice() : [];
  // undefined means normal in-process Claude and may use the configured
  // daemon home; null means a failed mapped-user resolution and must fail
  // closed without constructing any personal bridges.
  if (homeDir === null) return plugins;
  var effectiveHome = homeDir === undefined ? require("../../config").REAL_HOME || os.homedir() : homeDir;
  var rootOptions = { homeDir: effectiveHome, env: env, scopedHome: !!homeDir };
  var roots = skillDiscovery.getSkillRoots(cwd, rootOptions);
  var codexRoot = roots.filter(function (root) { return root.source === "codex-user"; })[0];
  var codexOpts = codexRoot ? { homeDir: effectiveHome, source: codexRoot.path, name: "codex" } : null;
  if (codexRoot && codexRoot.path !== path.join(effectiveHome, ".codex", "skills")) {
    codexOpts.name = "codex-" + crypto.createHash("sha256").update(codexRoot.path).digest("hex").slice(0, 12);
  }
  var bridgePath = skillDiscovery.ensureClaudeCodexPlugin(codexOpts);
  if (bridgePath) plugins.push({ type: "local", path: bridgePath, skipMcpDiscovery: true });
  for (var i = 0; i < roots.length; i++) {
    if (roots[i].source === "codex-user" || roots[i].source === "claude-user" || roots[i].source === "claude-project") continue;
    var rootId = crypto.createHash("sha256").update(roots[i].source + "\0" + path.resolve(roots[i].path)).digest("hex").slice(0, 12);
    var sharedPath = skillDiscovery.ensureClaudeCodexPlugin({ homeDir: effectiveHome, source: roots[i].path, name: "shared-" + roots[i].source + "-" + rootId });
    if (sharedPath) plugins.push({ type: "local", path: sharedPath, skipMcpDiscovery: true });
  }
  return plugins;
}

function buildWorkerQueryOptions(queryOpts, claudeOpts, workerCwd) {
  var queryOptions = {
    cwd: workerCwd,
    userInputMode: queryOpts.userInputMode || "auto",
  };
  ...
  if (queryOpts.systemPrompt) queryOptions.systemPrompt = applySystemPromptSnapshot(queryOpts.systemPrompt, queryOpts.systemPromptSnapshot);
  ...
  queryOptions.plugins = queryOpts.skipSkills ? (claudeOpts.plugins || []) : getSharedSkillPlugins(claudeOpts.originalHome, claudeOpts.plugins, workerCwd, queryOpts.env);
  ...
  if (queryOpts.onModelSwitch) queryOptions.reportModelSwitches = true;
  if (queryOpts.preToolUsePolicy) queryOptions.clayPreToolUsePolicy = true;
  ...
  if (queryOpts.persistSession === false) queryOptions.persistSession = false;
...
function claudeToolInputSchema(inputSchema) {
  if (!inputSchema || inputSchema.type !== "object" || !inputSchema.properties) return inputSchema || {};
  // Claude's SDK bundles a Zod processor that is incompatible with Zod 4
  // schemas from the daemon's dependency tree. Prefer the compatibility
  // surface shipped by Zod 4; the fallback keeps older installations working.
  var zod;
  try { zod = require("zod/v3"); } catch (e) { zod = require("zod"); }
  var z = zod.z || zod;
  var shape = {};
  var required = inputSchema.required || [];
  var keys = Object.keys(inputSchema.properties);
  for (var i = 0; i < keys.length; i++) {
    var key = keys[i];
    var property = inputSchema.properties[key] || {};
    var field;
    var nullable = Array.isArray(property.type) && property.type.indexOf("null") !== -1;
    var propertyType = nullable ? property.type.filter(function (type) { return type !== "null"; })[0] : property.type;
    if (Array.isArray(property.enum) && property.enum.length) field = z.enum(property.enum);
    else if (propertyType === "integer") field = z.number().int();
    else if (propertyType === "number") field = z.number();
    else if (propertyType === "boolean") field = z.boolean();
    else if (propertyType === "array") field = z.array(z.any());
    else if (propertyType === "object") field = z.record(z.any());
    else field = z.string();
    if (nullable) field = field.nullable();
    if (typeof property.minimum === "number" && typeof field.min === "function") field = field.min(property.minimum);
    if (typeof property.maximum === "number" && typeof field.max === "function") field = field.max(property.maximum);
    if (property.description && typeof field.describe === "function") field = field.describe(property.description);
...
function flattenEvent(raw) {
  ...
  var base = {};
  if (raw.user_message_uuid) base.userMessageId = raw.user_message_uuid;
  if (Array.isArray(raw.user_message_uuids)) base.userMessageIds = raw.user_message_uuids.slice();
  if (raw.resume_reason) base.resumeReason = raw.resume_reason;
  if (Number.isInteger(raw.result_index)) base.resultIndex = raw.result_index;
  if (raw.local_command) base.localCommand = raw.local_command;
  ...
  if (raw.type === "system") {
    if (raw.subtype === "init") {
      ...
      base.permissionMode = raw.permissionMode || raw.permission_mode || null;
      return base;
    }
  ...
  if (raw.type === "result") {
    ...
    base.is_error = raw.is_error === true;
    if (Number.isInteger(raw.queued_turn_count) && raw.queued_turn_count >= 0) base.queuedTurnCount = raw.queued_turn_count;
...
function createQueryHandle(rawQuery, messageQueue, abortController) {
  var correlation = claudeCorrelation.createCorrelation();
  var closed = false;
  var handle = {
      ...
      return {
        next: function() {
          return rawIter.next().then(function(result) {
            ...
            return { value: correlation.result(result.value, flattenEvent(result.value)), done: false };
          }, function(err) {
            closed = true;
            throw err;
          });
    ...
    getSubmittedMessageCount: correlation.submitted,
    ...
    pushMessage: function(text, images) {
      ...
      var message = correlation.message(content);
      var accepted = messageQueue.push(message);
      if (accepted) correlation.accepted(message);
      return accepted;
    },
    ...
    getContextUsage: function(options) {
      if (rawQuery && typeof rawQuery.getContextUsage === "function") {
        return rawQuery.getContextUsage(claudeContextUsage.options(options));
      }
      return Promise.resolve(null);
    },
...
function createWorkerQueryHandle(worker, canUseTool, onElicitation, callMcpTool, onUserDialog, onUserInputRequest, onModelSwitch, preToolUsePolicy) {
  var contextRequests = claudeContextUsage.createRequests(worker);
  var correlation = claudeCorrelation.createCorrelation();
  ...
  var pendingRewinds = {};
  var pendingPermissionModes = {};
  var pendingMcpPermissionModes = {};

  function rejectPendingPermissionModes(error) {
    Object.keys(pendingPermissionModes).forEach(function(id) {
      var pending = pendingPermissionModes[id];
      delete pendingPermissionModes[id];
      clearTimeout(pending.timer);
      pending.reject(error);
    });
    Object.keys(pendingMcpPermissionModes).forEach(function(id) {
      var pending = pendingMcpPermissionModes[id];
      delete pendingMcpPermissionModes[id];
      clearTimeout(pending.timer);
      pending.reject(error);
    });
  }
  ...
  worker.onMessage(function(msg) {
    switch (msg.type) {
      case "sdk_event":
        pushToIter(correlation.result(msg.event, flattenEvent(msg.event)));
        break;
      ...
      case "permission_request":
        if (canUseTool) {
          canUseTool(msg.toolName, msg.input, {
            ...
            defaultToNo: msg.defaultToNo === true,
            suppressAlwaysAllowRule: msg.suppressAlwaysAllowRule === true,
      ...
      case "pre_tool_policy_request":
        if (preToolUsePolicy) {
          Promise.resolve().then(function() {
            return preToolUsePolicy(msg.toolName, msg.input || {}, { toolUseID: msg.toolUseId });
          }).then(function(result) {
            worker.send({ type: "pre_tool_policy_response", requestId: msg.requestId, result: result || null });
          }).catch(function(error) {
            worker.send({ type: "pre_tool_policy_response", requestId: msg.requestId, error: error.message || String(error) });
          });
        } else {
          worker.send({ type: "pre_tool_policy_response", requestId: msg.requestId, result: null });
        }
        break;

      case "ask_user_request":
        if (onUserInputRequest) {
          claudeUserInput.ask(onUserInputRequest, msg.input, {
            toolUseID: msg.toolUseId,
            signal: { addEventListener: function() {} },
          }).then(function(result) {
      ...
      case "elicitation_request":
        if (onUserInputRequest || onElicitation) {
          var workerElicitationRequest = {
          ...
          var workerElicitation = onUserInputRequest
            ? claudeUserInput.elicitation(onUserInputRequest, workerElicitationRequest, {
              requestId: msg.requestId,
              signal: { addEventListener: function() {} },
            })
            : onElicitation(workerElicitationRequest, {
            signal: { addEventListener: function() {} },
          });
          workerElicitation.then(function(result) {
      ...
      case "model_switched":
        if (onModelSwitch) onModelSwitch(msg.change);
        break;
      case "context_usage_response":
        contextRequests.receive(msg);
        break;
      case "permission_mode_result": {
        var modePending = pendingPermissionModes[msg.requestId];
        if (!modePending) break;
        delete pendingPermissionModes[msg.requestId];
        clearTimeout(modePending.timer);
        if (msg.error) modePending.reject(new Error(msg.error));
        else modePending.resolve();
        break;
      }
      case "mcp_permission_mode_result": {
        var mcpPending = pendingMcpPermissionModes[msg.requestId];
        if (!mcpPending) break;
        delete pendingMcpPermissionModes[msg.requestId];
        clearTimeout(mcpPending.timer);
        if (msg.error) mcpPending.reject(new Error(msg.error));
        else mcpPending.resolve(msg.result || null);
        break;
      }
      case "context_usage":
      case "model_changed":
      ...
      case "query_done":
        rejectPendingPermissionModes(new Error("Worker query ended while changing permission mode"));
      ...
      case "query_error": {
        rejectPendingPermissionModes(new Error(msg.error || "Worker exited while changing permission mode"));
  ...
  var handle = {
    ...
    pushMessage: function(text, images) {
      ...
      if (text) content.push({ type: "text", text: text });
      var userMsg = correlation.message(content);
      var accepted = worker.send({ type: "push_message", content: userMsg }) === true;
      if (accepted) correlation.accepted(userMsg);
      return accepted;
    },
    ...
    setPermissionMode: function(mode) {
      return new Promise(function(resolve, reject) {
        ...
        var timer = setTimeout(function() {
          if (!pendingPermissionModes[requestId]) return;
          delete pendingPermissionModes[requestId];
          reject(new Error("Timed out changing permission mode in worker."));
        }, worker.permissionModeTimeoutMs || 10000);
        pendingPermissionModes[requestId] = { resolve: resolve, reject: reject, timer: timer };
        try {
          if (worker.send({ type: "set_permission_mode", mode: mode, requestId: requestId }) !== true) {
            delete pendingPermissionModes[requestId]; clearTimeout(timer); reject(new Error("Worker is unavailable."));
          }
        } catch (error) {
          delete pendingPermissionModes[requestId]; clearTimeout(timer); reject(error);
        }
      });
    },
    ...
    setMcpPermissionModeOverride: function(serverName, mode) {
      return new Promise(function(resolve, reject) {
        var requestId = crypto.randomUUID();
        var timer = setTimeout(function() {
          if (!pendingMcpPermissionModes[requestId]) return;
          delete pendingMcpPermissionModes[requestId];
          reject(new Error("Timed out changing MCP permission mode in worker."));
        }, worker.permissionModeTimeoutMs || 10000);
        pendingMcpPermissionModes[requestId] = { resolve: resolve, reject: reject, timer: timer };
        try {
          if (worker.send({ type: "set_mcp_permission_mode_override", serverName: serverName, mode: mode, requestId: requestId }) !== true) {
            delete pendingMcpPermissionModes[requestId]; clearTimeout(timer); reject(new Error("Worker is unavailable."));
          }
        } catch (error) {
          delete pendingMcpPermissionModes[requestId]; clearTimeout(timer); reject(error);
        }
      });
    },

    getContextUsage: function(options) {
      return iterEnded ? Promise.resolve(null) : contextRequests.request(options);
    },
    ...
    close: function() {
      rejectPendingPermissionModes(new Error("Worker query closed while changing permission mode"));
      // End the iterator
      endIter();
...
function createClaudeAdapter(opts) {
  ...
  var _querySDK = (opts && opts.loadSDK) || loadSDK;
  ...
  var adapter = {
    vendor: "claude",
    userInputCapability: { mode: "native", native: true, transport: "AskUserQuestion/onElicitation" },
    ...
    init: async function(initOpts) {
      ...
      warmupOptions.plugins = getSharedSkillPlugins(initOpts && initOpts.originalHome, null, initOpts && initOpts.cwd, initOpts && initOpts.env);
    ...
    createToolServer: function(def) {
      ...
      for (var i = 0; i < def.tools.length; i++) {
        var t = def.tools[i];
        sdkTools.push(sdk.tool(t.name, t.description, claudeToolInputSchema(t.inputSchema), t.handler));
      }
    ...
    createQuery: async function(queryOpts) {
      ...
      var nativeUserInputHandler = queryOpts.userInputMode === "fallback" ? null : queryOpts.onUserInputRequest;
      if (queryOpts.canUseTool || nativeUserInputHandler) {
        sdkOptions.canUseTool = claudeUserInput.canUseTool(Object.assign({}, queryOpts, { onUserInputRequest: nativeUserInputHandler }));
      }
      attachPreToolUsePolicy(sdkOptions, queryOpts.preToolUsePolicy);
      if (nativeUserInputHandler) {
        sdkOptions.onElicitation = function (request, elicitOpts) { return claudeUserInput.elicitation(nativeUserInputHandler, request, elicitOpts); };
      } else if (queryOpts.userInputMode !== "fallback" && queryOpts.onElicitation) sdkOptions.onElicitation = queryOpts.onElicitation;
      ...
      sdkOptions.plugins = queryOpts.skipSkills ? (co.plugins || []) : getSharedSkillPlugins(co.originalHome, co.plugins, queryOpts.cwd || _cwd, queryOpts.env);

      claudeModelSwitch.attach(sdkOptions, queryOpts.onModelSwitch);
    ...
    generateTitle: async function(messages, opts) {
      return require("../title-query").generateTitle(adapter, messages, opts, {
        model: "haiku",
        persistSession: false,
        skipSkills: true,
        adapterOptions: { CLAUDE: {
          linuxUser: opts && opts.linuxUser,
          settingSources: [],
  ...
  async function createWorkerQuery(queryOpts, claudeOpts, linuxUser) {
    ...
    var workerUserInputHandler = queryOpts.userInputMode === "fallback" ? null : queryOpts.onUserInputRequest;
    var workerElicitationHandler = queryOpts.userInputMode === "fallback" ? null : queryOpts.onElicitation;
    var handle = createWorkerQueryHandle(worker, queryOpts.canUseTool, workerElicitationHandler, queryOpts.callMcpTool, queryOpts.onUserDialog, workerUserInputHandler, queryOpts.onModelSwitch, queryOpts.preToolUsePolicy);
    ...
    var queryOptions = buildWorkerQueryOptions(queryOpts, claudeOpts, workerCwd);
...
module.exports = {
  ...
  contractTestKit: {
    ...
    buildWorkerQueryOptions: buildWorkerQueryOptions,
    normalizeEvent: flattenEvent,
    attachPreToolUsePolicy: attachPreToolUsePolicy,
  },
};
