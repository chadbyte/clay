...
var CODEX_QUERY_BOUND_TOOL_CAPABILITY = { mode: "dynamic", resume: "session-mcp-recovery" };
...
function flattenEvent(notification, state) {
  ...
  if (method === "turn/started") {
    ...
    state.currentTurnId = params.turnId || params.turn_id || (params.turn && params.turn.id) || null;
  ...
  if (method === "turn/completed") {
    ...
    // Generic usage reporting is independent from current context occupancy.
    // The latter is supplied only by a valid thread/tokenUsage snapshot.
    // Some normal protocol completions omit usage even though the preceding
    // token snapshot carried useful per-turn accounting. Keep that fallback
    // for the generic usage panel, but never use it as context occupancy.
    var inputTokens = usage && typeof usage.input_tokens === "number"
      ? usage.input_tokens : (typeof state.lastInputTokens === "number" ? state.lastInputTokens : 0);
    var outputTokens = usage && typeof usage.output_tokens === "number"
      ? usage.output_tokens : (typeof state.lastOutputTokens === "number" ? state.lastOutputTokens : 0);
    var cachedTokens = usage && typeof usage.cached_input_tokens === "number"
      ? usage.cached_input_tokens : (typeof state.lastCachedTokens === "number" ? state.lastCachedTokens : 0);
    ...
    var contextSnapshot = { valid: false };
    if (state.currentContextUsage
        && typeof state.currentContextUsage.input_tokens === "number"
        && typeof state.currentContextUsage.contextWindow === "number"
        && state.currentContextUsage.contextWindow > 0) {
      contextSnapshot = {
        valid: true,
        inputTokens: state.currentContextUsage.input_tokens,
        contextWindow: state.currentContextUsage.contextWindow,
      };
      if (state.currentContextUsage.turnId) contextSnapshot.turnId = state.currentContextUsage.turnId;
    }
    resultModelUsage[state.model] = {
    ...
    events.push({
      ...
      status: turnStatus,
      error: params.error || (params.turn && params.turn.error) || null,
      cost: null,
      duration: null,
      usage: hasTokenData ? {
  ...
  if (method === "thread/tokenUsage/updated") {
    ...
    if (tu) {
      var tokenTurnId = params.turnId || params.turn_id || tu.turnId || null;
      if (tokenTurnId && state.currentTurnId && tokenTurnId !== state.currentTurnId) {
        state.lastInputTokens = null;
        state.lastCachedTokens = null;
        state.lastOutputTokens = null;
        state.currentContextUsage = null;
      ...
      if (turnUsage) {
        ...
        currentWindow = typeof tu.modelContextWindow === "number" && isFinite(tu.modelContextWindow) && tu.modelContextWindow > 0
          ? tu.modelContextWindow : null;
        state.lastInputTokens = candidateInput !== null && isFinite(candidateInput) && candidateInput >= 0 && currentWindow !== null && candidateInput <= currentWindow ? candidateInput : null;
        state.lastCachedTokens = turnUsage.cachedInputTokens || 0;
        state.lastOutputTokens = turnUsage.outputTokens || 0;
        if (state.lastInputTokens !== null && currentWindow !== null) {
          state.currentContextUsage = { input_tokens: state.lastInputTokens, contextWindow: currentWindow };
          if (tokenTurnId || state.currentTurnId) state.currentContextUsage.turnId = tokenTurnId || state.currentTurnId;
        } else {
          state.currentContextUsage = null;
        }
      } else {
      ...
      if (currentWindow !== null) state.modelContextWindow = currentWindow;
    }
    return events;
  }
...
function createCodexQueryHandle(appServer, queryOpts) {
  ...
  async function runQueryLoop(initialMessage) {
    ...
    try {
      ...
      var threadResult;
      if (queryOpts.resumeSessionId) {
        // ThreadResumeParams has no dynamicTools field, but its supported
        // config override can start an exact-query MCP bridge without making
        // a replacement thread or changing the saved conversation history.
        var resumeParams = {
        ...
        if (queryOpts.sessionMcpServer && !queryOpts.isolatedRecovery) {
          resumeParams.config = { mcp_servers: {} };
          resumeParams.config.mcp_servers[queryOpts.sessionMcpServer.name] = {
            command: queryOpts.sessionMcpServer.command,
            args: queryOpts.sessionMcpServer.args || [],
            env: queryOpts.sessionMcpServer.env || {},
          };
        }
      ...
      while (!isCancelled()) {
        ...
        var availableSkills = queryOpts.skipSkills ? [] : skillDiscovery.discoverSkills(queryOpts.cwd, queryOpts.skillOptions || { env: queryOpts.env });
  ...
  var handle = {
    ...
    getContextUsage: function() {
      return Promise.resolve(state.currentContextUsage || {
        unavailable: true,
        reason: "current_context_unavailable",
      });
    },
...
function createCodexAdapter(opts) {
  ...
  var _activeQueries = [];
  var _recoveryHandoff = null;
  var _recoveryRuntimes = [];
  var _recoveryCleanup = Promise.resolve();
  var _recoveryCleanupTasks = [];

  function scopedSkillOptions(env) {
    if (!_runtimeLinuxUser) return { env: env };
    try {
      return Object.assign(skillDiscovery.optionsForOsUser(_resolveOsUserInfo(_runtimeLinuxUser)), { env: env });
    } catch (e) {
      return { disable: true };
    }
  }
  ...
  function waitForRecoveryHandoff() {
    return _recoveryHandoff || Promise.resolve();
  }

  function retainRecoveryRuntime(runtime) {
    _recoveryRuntimes.push(runtime);
  }

  function removeRecoveryRuntime(runtime) {
    var next = [];
    for (var i = 0; i < _recoveryRuntimes.length; i++) {
      if (_recoveryRuntimes[i] !== runtime) next.push(_recoveryRuntimes[i]);
    }
    _recoveryRuntimes = next;
  }

  function cleanupRecoveryRuntime(runtime) {
    for (var i = 0; i < _recoveryCleanupTasks.length; i++) {
      if (_recoveryCleanupTasks[i].runtime === runtime) return _recoveryCleanupTasks[i].promise;
    }
    var task = { runtime: runtime, promise: null };
    _recoveryCleanupTasks.push(task);
    var priorCleanup = _recoveryCleanup;
    task.promise = Promise.resolve(priorCleanup).catch(function() {
      return null;
    }).then(function() {
      return runtime.shutdown();
    }).catch(function(error) {
      console.error("[yoke/codex] Isolated recovery runtime shutdown failed:", error.message || error);
    }).then(function() {
      removeRecoveryRuntime(runtime);
      var remainingTasks = [];
      for (var ti = 0; ti < _recoveryCleanupTasks.length; ti++) {
        if (_recoveryCleanupTasks[ti] !== task) remainingTasks.push(_recoveryCleanupTasks[ti]);
      }
      _recoveryCleanupTasks = remainingTasks;
    });
    _recoveryCleanup = task.promise;
    return task.promise;
  }

  function shutdownRecoveryRuntimes() {
    var runtimes = _recoveryRuntimes.slice();
    if (!runtimes.length) return _recoveryCleanup;
    return Promise.all(runtimes.map(function(runtime) {
      return cleanupRecoveryRuntime(runtime);
    })).then(function() {
      _recoveryRuntimes = [];
    });
  }

  async function createIsolatedRecoveryQuery(queryOpts) {
    var releaseHandoff;
    var priorHandoff = _recoveryHandoff;
    var myHandoff = new Promise(function(resolve) { releaseHandoff = resolve; });
    _recoveryHandoff = myHandoff;
    await (priorHandoff || Promise.resolve());
    try {
      await _recoveryCleanup;
      if (_refCount > 0 || _initializing) {
        throw new Error("Codex resumed-tool recovery is waiting for an active shared runtime; retry after its current queries finish.");
      }
      if (_appServer) {
        var stopped = await adapter.shutdownIfIdle(0);
        if (!stopped) throw new Error("Codex resumed-tool recovery could not safely release the shared runtime; retry after its current queries finish.");
      }
      var recoveryRuntime = createCodexAdapter(Object.assign({}, _defaultInitOpts, {
        recoveryRuntime: true,
        recoverySessionMcpServer: queryOpts.sessionMcpServer,
      }));
      retainRecoveryRuntime(recoveryRuntime);
      var recoveryOpts = Object.assign({}, queryOpts, {
        isolatedRecovery: true,
        onRuntimeFinished: function() {
          return cleanupRecoveryRuntime(recoveryRuntime);
        },
      });
      try {
        return await recoveryRuntime.createQuery(recoveryOpts);
      } catch (error) {
        await cleanupRecoveryRuntime(recoveryRuntime);
        throw error;
      }
    } finally {
      releaseHandoff();
      if (_recoveryHandoff === myHandoff) _recoveryHandoff = null;
    }
  }
  ...
  function buildReadyResponse(skillNames) {
    return {
      ...
      capabilities: {
        ...
        queryBoundTools: CODEX_QUERY_BOUND_TOOL_CAPABILITY,
        skillSharing: true,
  ...
  var adapter = {
    vendor: "codex",
    queryBoundToolCapability: CODEX_QUERY_BOUND_TOOL_CAPABILITY,
    ...
    init: function(initOpts) {
      ...
      var initPromise = (async function() {
        ...
        var mcpServerConfig = Object.assign({}, serverOpts.config.mcp_servers || {});
        try {
        ...
        // Recovery uses a dedicated app-server so the query-local bridge can
        // coexist with every server already configured for this runtime.
        var recoveryMcpServer = effectiveInitOpts.recoverySessionMcpServer;
        if (recoveryMcpServer && recoveryMcpServer.name && recoveryMcpServer.command) {
          mcpServerConfig[recoveryMcpServer.name] = {
            command: recoveryMcpServer.command,
            args: recoveryMcpServer.args || [],
            env: recoveryMcpServer.env || {},
          };
        }
        ...
        var skillNames = [];
        try {
          var skillOptions = scopedSkillOptions(effectiveInitOpts.env);
          var roots = skillOptions.disable ? [] : skillDiscovery.getSkillRoots(_cwd, skillOptions);
          var extraRootPaths = [];
          ...
          var sharedSkills = skillDiscovery.discoverSkills(_cwd, skillOptions);
    ...
    createQuery: async function(queryOpts) {
      ...
      if (queryOpts && queryOpts.sessionMcpServer && queryOpts.resumeSessionId && !queryOpts.isolatedRecovery) {
        return createIsolatedRecoveryQuery(queryOpts);
      }
      await waitForRecoveryHandoff();
      ...
      var handleOpts = {
        ...
        sessionMcpServer: queryOpts.sessionMcpServer || null,
        isolatedRecovery: queryOpts.isolatedRecovery === true,
        registerBackgroundTaskReset: registerBackgroundTaskReset,
        ephemeral: queryOpts.ephemeral === true,
        skillOptions: scopedSkillOptions(queryOpts.env),
      };
      ...
      try {
        handleOpts.onFinished = function() {
          ...
          decrementRefCount();
          if (typeof queryOpts.onRuntimeFinished === "function") queryOpts.onRuntimeFinished();
        };
    ...
    generateTitle: async function(messages, opts) {
      ...
      var model = models.indexOf("gpt-5.6-luna") !== -1 ? "gpt-5.6-luna"
        : (opts && models.indexOf(opts.model) !== -1 ? opts.model : models[0]);
      if (!model) throw new Error("No supported Codex title model is available");
      return require("../title-query").generateTitle(adapter, messages, opts, {
        model: model,
        effort: "low",
        ephemeral: true,
        adapterOptions: { CODEX: { sandboxMode: "read-only", approvalPolicy: "never" } },
      });
    },
    ...
    shutdownIfIdle: function(idleMs) {
      ...
      if (_initializing) return Promise.resolve(false);
