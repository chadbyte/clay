...
var claudeContextUsage = require(path.join(__dirname, "../claude-context-usage"));
var claudeModelSwitch = require(path.join(__dirname, "../claude-model-switch"));
var createPolicyBroker = require("./claude-worker-policy").createPolicyBroker;
var fs = require("fs");
...
function buildZodShape(z, inputSchema) {
  ...
  for (var i = 0; i < keys.length; i++) {
    ...
    var prop = props[key];
    var field;
    var nullable = Array.isArray(prop.type) && prop.type.indexOf("null") !== -1;
    var propType = nullable ? prop.type.filter(function (type) { return type !== "null"; })[0] : prop.type;
...
function buildMcpServersFromDescriptors(descriptors, sdk) {
  ...
  var z;
  try { z = require("zod/v3"); } catch (e) {
    try { z = require("zod").z; } catch (e2) {
      try { z = require("zod"); } catch (e3) { return null; }
    }
  }
...
var preToolPolicyBroker = createPolicyBroker({ send: sendToDaemon, timeoutMs: 10000 });
...
function handleMessage(msg) {
  ...
  switch (msg.type) {
    ...
    case "get_context_usage":
      claudeContextUsage.respond(queryInstance, msg, sendToDaemon);
      break;
    case "set_model":
      handleSetModel(msg);
      break;
    ...
    case "permission_response":
      ...
      break;
    case "pre_tool_policy_response":
      handlePreToolPolicyResponse(msg);
      break;
...
function handlePreToolPolicyResponse(msg) {
  preToolPolicyBroker.respond(msg);
}

function preToolUsePolicy(toolName, input, toolUseId, signal) {
  var requestId = crypto.randomUUID();
  return preToolPolicyBroker.request({ type: "pre_tool_policy_request", requestId: requestId, toolName: toolName,
    input: input || {}, toolUseId: toolUseId || "" }, signal);
}
...
async function handleQueryStart(msg) {
  ...
  var nativeUserInput = options.userInputMode !== "fallback";
  delete options.userInputMode;
  var usePreToolPolicy = options.clayPreToolUsePolicy === true;
  delete options.clayPreToolUsePolicy;
  if (usePreToolPolicy) {
    options.hooks = options.hooks || {};
    options.hooks.PreToolUse = (options.hooks.PreToolUse || []).concat([{
      hooks: [async function(input, toolUseId, hookOpts) {
        input = input || {};
        var decision = await preToolUsePolicy(input.tool_name, input.tool_input, toolUseId || input.tool_use_id, hookOpts && hookOpts.signal);
  ...
  options.canUseTool = function(toolName, input, toolOpts) {
    ...
    if (toolName === "AskUserQuestion" && nativeUserInput) {
    ...
    if (toolName === "AskUserQuestion") {
      return Promise.resolve({ behavior: "deny", message: "Use the query's structured input fallback tool." });
    }
  ...
  if (options.reportModelSwitches) {
    claudeModelSwitch.attach(options, function(change) { sendToDaemon({ type: "model_switched", change: change }); });
  }
  delete options.reportModelSwitches;
  ...
  try {
    ...
    abortController = null;
    pendingPermissions = {};
    preToolPolicyBroker.clear("Permission policy query ended.");
    pendingAskUser = {};
    pendingElicitations = {};
  }
}
...
async function handleSetPermissionMode(msg) {
  if (!queryInstance) { sendToDaemon({ type: "permission_mode_result", requestId: msg.requestId, error: "No active query." }); return; }
  try {
    await queryInstance.setPermissionMode(msg.mode);
    sendToDaemon({ type: "permission_mode_result", requestId: msg.requestId });
  } catch (e) {
    sendToDaemon({ type: "permission_mode_result", requestId: msg.requestId, error: "Failed to set permission mode: " + (e.message || e) });
  }
}
...
async function handleSetMcpPermissionModeOverride(msg) {
  if (!queryInstance || typeof queryInstance.setMcpPermissionModeOverride !== "function") {
    sendToDaemon({ type: "mcp_permission_mode_result", requestId: msg.requestId, error: "MCP permission controls are unavailable." });
    return;
  }
  try {
    var result = await queryInstance.setMcpPermissionModeOverride(msg.serverName, msg.mode);
    sendToDaemon({ type: "mcp_permission_mode_result", requestId: msg.requestId, result: result || null });
  } catch (e) {
    sendToDaemon({ type: "mcp_permission_mode_result", requestId: msg.requestId, error: e.message || String(e) });
  }
}
...
function cleanup() {
  preToolPolicyBroker.clear("Permission policy worker closed.");
  if (_keepAlive) {
