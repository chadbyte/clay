...
function validateOpenCodeConfig(config) {
  if (!config || typeof config !== "object" || Array.isArray(config)) {
    throw new Error("OpenCode resolved configuration is malformed");
  }
  var globalPermission = config.permission;
  var globalAsk = globalPermission === "ask" || (
    isSafeOpenCodePermission(globalPermission) && globalPermission["*"] === "ask"
  );
  if (!globalAsk) {
    throw new Error("OpenCode resolved configuration does not preserve global ask permissions");
  }
  if (config.agent !== undefined && (!config.agent || typeof config.agent !== "object" || Array.isArray(config.agent))) {
    throw new Error("OpenCode resolved configuration has malformed agents");
  }
  ...
  for (var i = 0; i < names.length; i++) {
    ...
    if (!agent || typeof agent !== "object" || Array.isArray(agent)) {
      throw new Error("OpenCode agent has malformed resolved configuration: " + names[i]);
    }
