...
function createProjectContext(opts) {
  ...
  // --- Mate session map (mateId → sessionLocalId) ---
  var mateSessions = {};

  function getOrCreateMateSession(mateId, ws) {
    // Check if we already have a session for this mate
    if (mateSessions[mateId]) {
      var existing = sm.sessions.get(mateSessions[mateId]);
      if (existing) return existing;
    }

    // Search existing sessions for a mate session
    for (var entry of sm.sessions) {
      var s = entry[1];
      if (s.mateId === mateId) {
        mateSessions[mateId] = s.localId;
        return s;
      }
    }

    // Create new session for this mate
    var mate = matesModule.getMate(mateId);
    var mateName = (mate && mate.profile && mate.profile.displayName) || (mate && mate.name) || "New Mate";
    var session = sm.createSession({ ownerId: ws._clayUser ? ws._clayUser.id : null, sessionVisibility: "private" }, ws);
    session.title = "Mate: " + mateName;
    session.mateId = mateId;
    sm.saveSessionFile(session);
    mateSessions[mateId] = session.localId;
    return session;
  }
  ...
  function handleMessage(ws, msg) {
    ...
    // --- Mate DM: route through SDK (Claude Code session) ---
    if (msg.type === "mate_dm_send") {
      var mateId = msg.mateId;
      var text = msg.text;
      if (!mateId || !text) return;

      var mate = matesModule.getMate(mateId);
      if (!mate) return;

      var userId = ws._clayUser ? ws._clayUser.id : null;
      var dmKey = "mate:" + mateId;

      // Save user message to DM history
      var userDmMsg = dm.sendMessage(dmKey, userId, text);
      // Send confirmation back so it appears in DM UI
      ws.send(JSON.stringify({ type: "dm_message", dmKey: dmKey, message: userDmMsg }));

      // Get or create the mate's Claude Code session
      var session = getOrCreateMateSession(mateId, ws);
      ...
      // Track response text for saving as mate DM message when done
      session._mateDmResponseText = "";
      session._mateDmKey = dmKey;
      session._mateDmWs = ws;
      session._mateDmMateId = mateId;

      // Set callback to save complete response as mate DM message
      session.onQueryComplete = function (s) {
        var responseText = (s._mateDmResponseText || "").trim();
        if (responseText && s._mateDmKey) {
          // Save mate's response to DM history
          var mateDmMsg = dm.sendMessage(s._mateDmKey, s._mateDmMateId, responseText);
          // Send to client
          if (s._mateDmWs && s._mateDmWs.readyState === 1) {
            s._mateDmWs.send(JSON.stringify({ type: "dm_message", dmKey: s._mateDmKey, message: mateDmMsg }));
          }
        }
        s._mateDmResponseText = "";
        s._mateDmKey = null;
        s._mateDmWs = null;
        s._mateDmMateId = null;
        s.onQueryComplete = null;
      };

      // Start or continue SDK query
      if (!session.isProcessing) {
        session.isProcessing = true;
        onProcessingChanged();
        session.sentToolResults = {};
        if (!session.queryInstance && !session.worker) {
          sdk.startQuery(session, text, undefined, getLinuxUserForSession(session));
        } else {
