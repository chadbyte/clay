  ...
  function handleMessage(ws, msg) {
    ...
    if (msg.type === "ralph_wizard_complete") {
      ...
      if (wData.mode === "own" && wData.promptText) {
        ...
        if (wData.judgeText) {
          // Both provided: write JUDGE.md and go straight to approval
        ...
        // Only PROMPT provided: craft JUDGE only
        startClaudeDirWatch();

        var judgeCraftPrompt = "Use the /clay-ralph skill to create ONLY a JUDGE.md file for the following Ralph Loop. " +
          "The PROMPT.md has already been written by the user. Read it, then create a matching JUDGE.md " +
          "with clear pass/fail evaluation criteria.\n\n" +
          "## Loop Directory\n" + lDir;
        ...
        craftingSession.history.push({ type: "user_message", text: judgeCraftPrompt });
        sm.appendToSessionFile(craftingSession, { type: "user_message", text: judgeCraftPrompt });
        sendToSession(craftingSession.localId, { type: "user_message", text: judgeCraftPrompt });
        ...
        sdk.startQuery(craftingSession, judgeCraftPrompt, undefined, getLinuxUserForSession(craftingSession));
