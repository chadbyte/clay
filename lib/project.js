...
function createProjectContext(opts) {
  ...
  var _sessionPair = attachSessionPair({
    ...
    onProcessingChanged: onProcessingChanged,
    adapters: adapters,
    onPartnerResult: function (driver, worker, token) { if (_autonomousRun) _autonomousRun.onPartnerResult(driver, worker, token); },
  });
    ...
    return {
      listTools: function () {
        ...
        if (boundSession) {
          ...
          var scheduledTaskTools = _scheduledTasks ? _scheduledTasks.getToolDefs(boundSession) : [];
          for (var stti = 0; stti < scheduledTaskTools.length; stti++) {
            tools.push({ server: "clay-scheduled-tasks", name: scheduledTaskTools[stti].name, description: scheduledTaskTools[stti].description, inputSchema: normalizeToolSchema(scheduledTaskTools[stti].inputSchema) });
          }
      ...
      callTool: function (serverName, toolName, args) {
        ...
        if (boundSession && serverName === "clay-scheduled-tasks") {
          var scheduledTaskTools = _scheduledTasks ? _scheduledTasks.getToolDefs(boundSession) : [];
          for (var stti = 0; stti < scheduledTaskTools.length; stti++) {
            if (scheduledTaskTools[stti].name === toolName && typeof scheduledTaskTools[stti].handler === "function") return Promise.resolve(scheduledTaskTools[stti].handler(args || {}));
          }
        }
