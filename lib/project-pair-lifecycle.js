...
var lifecycleStatus = require("./project-pair-lifecycle-status");
var contextStatus = lifecycleStatus.contextStatus;
var firstNumber = lifecycleStatus.firstNumber;
var continuityStatus = lifecycleStatus.continuityStatus;
var activityStatus = lifecycleStatus.activityStatus;
var clampText = lifecycleStatus.clampText;
...
function attachPairLifecycle(ctx) {
  ...
  function ledgerFor(driver, selectedWorker) {
    ...
    var fallbackId = normalized.ok && normalized.kind !== "adhoc" && normalized.workerIds.length === 1 ? normalized.workerIds[0] : null;
    var worker = selectedWorker || fallbackId != null && sm.sessions.get(fallbackId);
  ...
  function recordGenerationStart(driver, worker) {
    var ledger = ledgerFor(driver, worker);
  ...
  function resolveDriverPair(caller) {
    ...
    var targetOptions = Object.assign({}, options || { mutation: true }, {
      multiWorkerFeature: ctx.multiWorkerFeature,
    });
    var resolved = pairTarget.resolveGroupTarget(sm, group, caller, args, targetOptions);
    var worker = resolved.partner;
    if ((caller.ownerId || null) !== (worker.ownerId || null)) throw new Error("split partner access denied");
    return { group: resolved.group, worker: resolved.partner, role: resolved.role, workerId: resolved.workerId };
  }
  ...
  function partnerStatus(caller, args) {
    var group = caller && store.groupForMember(caller.localId);
    var roles = splitRoles.normalizePair(group && group.pair, group && group.members);
    if ((!args || args.workerId == null) && roles.ok && roles.kind !== "adhoc" && roles.workerIds.length > 1) {
      var workers = roles.workerIds.map(function (workerId) {
        return partnerStatus(caller, { workerId: workerId });
      });
      return { driverId: caller.localId, workerIds: roles.workerIds.slice(), workers: workers,
        instruction: "Pass the selected workerId to targeted partner tools. Call partner_status without workerId to rediscover current Workers." };
    }
    var resolved = resolveDriverPair(caller, args, { readOnly: true });
    ...
    var ledger = ledgerFor(caller, worker).map(function (record) {
      return {
  ...
  function sourcePairRegistered(caller, worker) {
    try {
      ...
      var group = store.groupForMember(caller.localId);
      var normalized = splitRoles.normalizePair(group && group.pair, group && group.members);
      return !!(normalized.ok && normalized.kind !== "adhoc" && normalized.driverId === caller.localId &&
        normalized.workerIds.indexOf(worker.localId) !== -1);
  ...
  function replacementTargetRegistered(caller, oldWorker, sourceWorkerIds) {
    try {
      if (sm.sessions.get(caller.localId) !== caller || sm.sessions.get(oldWorker.localId) !== oldWorker) return null;
      var group = store.groupForMember(caller.localId);
      var normalized = splitRoles.normalizePair(group && group.pair, group && group.members);
      if (!normalized.ok || normalized.kind === "adhoc" || normalized.driverId !== caller.localId) return null;
      var candidates = normalized.workerIds.filter(function (id) { return sourceWorkerIds.indexOf(id) === -1; });
      if (candidates.length !== 1) return null;
      var target = sm.sessions.get(candidates[0]);
  ...
  function replacePartner(args, caller) {
    var resolved = resolveDriverPair(caller, args, { mutation: true });
    var group = resolved.group;
    var oldWorker = resolved.worker;
    var versioned = resolved.role.kind === "versioned";
    var sourceWorkerIds = resolved.role.workerIds.slice();
    ledgerFor(caller);
    ...
    try {
      ...
      var createReplacement = versioned ? ctx.replaceWorkerForDriver : ctx.createWorkerForDriver;
      created = createReplacement(caller, versioned ? oldWorker : {
        ...
        workerEffort: typeof args.workerEffort === "string" ? args.workerEffort : "",
        expectedGroup: group,
        expectedWorkerIds: sourceWorkerIds,
      } : undefined);
    } catch (e) {
      var target = replacementTargetRegistered(caller, oldWorker, sourceWorkerIds);
      if (target) {
    ...
    return Promise.resolve(ctx.sendToPartner({ message: message, wait: args.wait, timeoutSeconds: args.timeoutSeconds, workerId: created.worker.localId }, caller))
  ...
  function recordEvaluation(args, caller) {
    var resolved = resolveDriverPair(caller, args, { mutation: true });
    var ledger = ledgerFor(caller, resolved.worker);
  ...
  function optionalStatus(boundSession, args) {
    try { return partnerStatus(boundSession, args); } catch (e) { return null; }
  }

  function toolHandlers(boundSession) {
    return {
      status: function (args) {
        try { return toolResult(partnerStatus(boundSession, args || {})); }
