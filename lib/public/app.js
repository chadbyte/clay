...
import { initLoopInterview } from './modules/loop-interview.js';
...
import { initScheduledTasks, closeScheduledTasks } from './modules/scheduled-tasks.js';
...
import { initProjectLogs, closeProjectLogs } from './modules/project-logs.js';
...
import { initMisc, flushPendingExtMessages, showImageModal as _miscShowImageModal, closeImageModal as _miscCloseImageModal, showConfirm as _miscShowConfirm, hideConfirm as _miscHideConfirm, showForceChangePinOverlay as _miscShowForceChangePinOverlay, sendExtensionCommand as _miscSendExtensionCommand, handleExtensionResult as _miscHandleExtensionResult } from './modules/app-misc.js';
import { showPasteModal } from './modules/paste-modal.js';
...
// --- DOM refs ---
  ...
  // --- Confirmation modal -> modules/app-misc.js
  ...
  createStore({
    ...
    autonomousSelectedMode: "normal",
    autonomousArming: false,
    autonomousArmToken: null,
    autonomousArmRequestId: null,
    autonomousArmSessionId: null,
    autonomousActionRequestId: null,
    autonomousActionSessionId: null,
    autonomousCriteria: "",
    autonomousMaxContinuations: 10,
    autonomousMaxMinutes: 60,
    autonomousRun: null,
    autonomousExpanded: false,
    loopInterviewBrief: null,
    loopInterviewEditing: false,
    loopInterviewRunRequestId: null,
    loopInterviewRunSessionId: null,
    loopInterviewStarting: false,
    loopInterviewRequestId: null,
    loopInterviewSessionId: null,
    slashCommands: [],
    processing: false,
    ...
    projectLogsOpen: false,
    projectLogsWide: false,
    projectLogsFullscreen: false,
    projectLogsEntries: [],
    ...
    scheduledTasksWide: false,
    scheduledTasksFullscreen: false,
    scheduledTaskRecords: [],
    scheduledTaskDraft: null,
    scheduledTaskEditingId: null,
    scheduledTaskNewSessionPending: false,
    scheduledTaskInterviewRequestId: null,
    scheduledTaskActionRequestId: null,
    scheduledTaskListRequest: null,
    scheduledTaskInterviewId: null,
    scheduledTaskExpandedId: null,
    scheduledTaskDeleteId: null,
    scheduledTaskDraftForm: null,
    scheduledTaskEditForm: null,
    scheduledTaskCreateMenuOpen: false,
    scheduledTaskManualRequest: null,
    installedTools: [],
    capsulesEnabled: false,
    capsulesPreferenceLoaded: false,
  ...
  fetch("/api/me").then(function (r) { return r.json(); }).then(function (d) {
    ...
    if (_perms) {
      ...
      if (!_perms.fileBrowser) {
        ...
        var gitPlacardEl = document.getElementById("git-placard");
        if (gitPlacardEl) gitPlacardEl.style.display = "none";
      }
      if (!_perms.skills) {
      ...
      if (!_perms.scheduledTasks) {
        var schToolBtn = document.getElementById("scheduler-btn");
        if (schToolBtn) schToolBtn.style.display = "none";
      }
  ...
  // --- Sticky Notes ---
  // The canvas reads the socket from ws-ref and the connection flag from the
  // store directly, so it needs no injected context.
  initStickyNotes();
  initNotesBrowser();
  ...
  registerExclusiveClosers([closeProjectLogs, closeScheduledTasks, closeFileViewer, closeTerminal]);
  ...
  if (fileBrowserBtn) fileBrowserBtn.addEventListener("click", function () { closeProjectLogs(); closeScheduledTasks(); if (isNotesBrowserOpen()) closeNotesBrowser(); if (isSchedulerOpen()) closeScheduler(); });
  if (gitPlacardMoreBtn) gitPlacardMoreBtn.addEventListener("click", function () { closeProjectLogs(); closeScheduledTasks(); if (isNotesBrowserOpen()) closeNotesBrowser(); if (isSchedulerOpen()) closeScheduler(); });
  if (terminalSidebarBtn) terminalSidebarBtn.addEventListener("click", function () { closeProjectLogs(); closeScheduledTasks(); if (isNotesBrowserOpen()) closeNotesBrowser(); if (isSchedulerOpen()) closeScheduler(); });

  // --- Legacy scheduled Loop UI ---
