...
// --- DOM refs ---
  ...
  function buildMateInterviewPrompt(mate) {
    ...
    if (sd.communicationStyle) {
      var dir = sd.communicationStyle.directness;
      var det = sd.communicationStyle.detail;
      var dirLabel = dir > 0.6 ? "direct" : dir < 0.4 ? "soft" : "balanced";
      var detLabel = det > 0.6 ? "detailed" : det < 0.4 ? "concise" : "balanced";
      parts.push("Communication: " + dirLabel + ", " + detLabel);
    }
  ...
  function handleDmSend() {
    ...
    if (dmTargetUser && dmTargetUser.isMate) {
      ws.send(JSON.stringify({ type: "mate_dm_send", mateId: dmTargetUser.id, text: text }));
    } else {
  ...
  function processMessage(msg) {
      switch (msg.type) {
        ...
        case "dm_history":
          ...
          if (pendingMateInterview && msg.targetUser && msg.targetUser.isMate) {
            var interviewText = buildMateInterviewPrompt(pendingMateInterview);
            pendingMateInterview = null;
            if (ws && ws.readyState === 1) {
              ws.send(JSON.stringify({ type: "mate_dm_send", mateId: msg.targetUser.id, text: interviewText }));
            }
          }
          break;
