...
// --- DOM refs ---
  ...
  function blinkIO() {
    ...
    // If in mate DM, also blink the active mate's icon strip dot
    var mateDot = null;
    if (dmMode && dmTargetUser && dmTargetUser.isMate) {
      mateDot = document.querySelector('.icon-strip-mate.active .icon-strip-status');
      if (mateDot) mateDot.classList.add("io");
    }
    ...
    ioTimer = setTimeout(function () {
      ...
      if (mateDot) mateDot.classList.remove("io");
    }, 80);
  }
