...
function scheduleHydrationRetry(requestId) {
  hydrationRecovery.schedule(requestId, function () {
    ...
    store.set({ pendingMessageQueue: Object.assign({}, currentQueue(), { loading: false, error: "Queue refresh timed out. Retrying…", errorSource: "hydration" }) });
...
export function requestPendingMessages(preserveError) {
  ...
  store.set({ pendingMessageQueue: Object.assign({}, currentQueue(), { projectSlug: target.projectSlug, sessionId: target.sessionId, loading: true, error: preserveError ? currentQueue().error : "", errorSource: preserveError ? currentQueue().errorSource : null }), pendingMessageQueueRecovery: true });
  if (!send({ type: "pending_message_get", projectSlug: target.projectSlug, sessionId: target.sessionId, requestId: requestId })) {
    forgetRequest(requestId);
    store.set({ pendingMessageQueue: Object.assign({}, currentQueue(), { loading: false, error: "Queue connection unavailable", errorSource: "hydration" }) });
    retryHydration();
    return false;
  }
...
function conflict(request, error) {
  var edit = store.get('pendingMessageQueueEdit');
  var update = { pendingMessageQueue: Object.assign({}, currentQueue(), { loading: false, error: error || "Queue changed. Refreshed the latest order.", errorSource: "mutation" }) };
...
export function handlePendingMessageQueueMessage(msg) {
  ...
  if (msg.type === "pending_message_result") {
    ...
    if (!result.ok) {
      if (request.kind === "get") {
        store.set({ pendingMessageQueue: Object.assign({}, currentQueue(), { loading: false, error: result.error || "Queue update failed", errorSource: "hydration" }) });
        retryHydration();
        return true;
      }
  ...
  var update = { pendingMessageQueue: Object.assign({}, queue, { projectSlug: msg.projectSlug, sessionId: msg.sessionId, revision: Number(msg.revision) || queue.revision, paused: msg.paused === true, items: items, loading: false, error: "", errorSource: null }) };
  if (nextEdit) update.pendingMessageQueueEdit = nextEdit;
  store.set(update);
  return true;
}
...
function render() {
  ...
  var visible = queueCanRender(queue, target) && (items.length > 0 || queue.errorSource === "mutation");
