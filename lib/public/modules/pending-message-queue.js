...
export function handlePendingMessageQueueMessage(msg) {
  ...
  if (msg.type === "pending_message_result") {
    ...
    if (request.kind === "edit") {
      ...
      if (currentEdit && currentEdit.id === request.itemId && currentEdit.text === request.editText && currentEdit.generation === request.editGeneration) {
  ...
  store.set({ pendingMessageQueue: Object.assign({}, queue, { projectSlug: msg.projectSlug, sessionId: msg.sessionId, revision: Number(msg.revision) || queue.revision, paused: msg.paused === true, items: items, loading: false, error: "" }) });
  return true;
}
...
function beginEdit(id) {
  ...
  for (var i = 0; i < items.length; i++) {
    if (items[i].id === id && items[i].state === "pending" && isOwn(items[i])) {
      store.set({ pendingMessageQueueEdit: { id: id, text: items[i].message.text || "", error: "", generation: nextEditGeneration() } });
      var row = findRow(id);
...
function saveEdit(id) {
  ...
  sendMutation("edit", { id: id, message: { text: edit.text } });
}
...
function renderItem(item, index) {
  ...
  if (isEditing) {
    ...
    html += '<div class="pending-message-edit-actions"><button type="button" data-action="edit-cancel">Cancel</button><button type="button" class="is-primary" data-action="edit-save">Save</button></div>';
  } else {
...
export function initPendingMessageQueue() {
  ...
  store.subscribe(function (state, previous) {
    ...
    var editNeedsRender = edit !== previousEdit && (!edit || !previousEdit || edit.id !== previousEdit.id || edit.error !== previousEdit.error);
