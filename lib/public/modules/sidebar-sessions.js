...
function confirmDeleteSession(session) {
  showConfirm(describeSessionDelete(session, cachedSessions).message, function () {
    var ws = getWs();
...
function appendSessionCloseButton(el, session) {
  ...
  closeBtn.addEventListener("click", function (e) {
    e.preventDefault();
    e.stopPropagation();
    // A deletion that removes or may remove Split Workers always gets the
    // explicit dialog instead of the two-click armed shortcut.
    if (describeSessionDelete(session, cachedSessions).requiresDialog) {
      clearArmedSessionDelete();
...
export function resolveDefaultVendor() {
  ...
  return resolvePreferredVendor(installed) || "claude";
}
...
function renderSplitGroupItem(group, members) {
  ...
  html += "</span>";
  var splitProcessing = false;
  var splitBackgroundTaskCount = 0;
  for (var si = 0; si < members.length; si++) {
    if (members[si].isProcessing) splitProcessing = true;
    splitBackgroundTaskCount += members[si].backgroundTaskCount || 0;
  }
  ...
  var unread = 0;
  for (var ui = 0; ui < members.length; ui++) unread += members[ui].unread || 0;
...
export function renderSessionList(sessions) {
  ...
  for (var sg = 0; sg < splitGroups.length; sg++) {
    ...
    var splitMembers = [];
    for (var smi = 0; smi < splitGroup.members.length; smi++) {
      var splitMember = sessionsById.get(splitGroup.members[smi]);
      if (splitMember) splitMembers.push(splitMember);
    }
    if (splitMembers.length !== splitGroup.members.length) continue;
    var hasHierarchyMember = false;
    for (var hmi = 0; hmi < splitMembers.length; hmi++) {
      if (hierarchySessionIds.has(splitMembers[hmi].id)) hasHierarchyMember = true;
    }
    if (hasHierarchyMember) continue;
    var splitLastActivity = 0;
    for (var lai = 0; lai < splitMembers.length; lai++) splitLastActivity = Math.max(splitLastActivity, splitMembers[lai].lastActivity || 0);
    items.push({
      type: "split-group",
      group: splitGroup,
      members: splitMembers,
      lastActivity: splitLastActivity,
    });
  }
