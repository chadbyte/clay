...
function formatState(state) {
  ...
  return labels[state] || "Until complete";
}
...
function renderSelector() {
  ...
  var modeLabel = state.autonomousArming ? "Enabling Until complete" : (state.autonomousSelectedMode === "until-complete" ? "Until complete" : "Normal");
...
export function handleAutonomousRunMessage(msg) {
  if (msg.type === "autonomous_run_arm_result") {
    ...
    if (!msg.ok) {
      ...
      showToast(msg.error || "Until complete could not be enabled.", "error");
    } else {
  ...
  if (msg.type === "autonomous_run_action_result") {
    ...
    store.set({ autonomousActionRequestId: null, autonomousActionSessionId: null });
    if (!msg.ok) showToast(msg.error || "The Until complete action failed.", "error");
    return true;
  }
