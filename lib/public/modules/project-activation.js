...
export function splitPanesMatchProject(split, slug) {
  if (!split || !Array.isArray(split.panes) || split.panes.length < 2 || split.panes.length > 3 || !slug) return false;
  for (var i = 0; i < split.panes.length; i++) if (split.panes[i].slug !== slug) return false;
  return true;
}
...
export function hasCurrentSplitGroup(split, groups) {
  ...
  if (!members || members.length < 2 || members.length > 3) return false;
  var available = Array.isArray(groups) ? groups : [];
  for (var i = 0; i < available.length; i++) {
    var group = available[i];
    if (group.id !== split.groupId || !Array.isArray(group.members) || group.members.length !== members.length) continue;
    var matches = true;
    for (var mi = 0; mi < members.length; mi++) if (group.members.indexOf(members[mi]) === -1) matches = false;
...
export function reconcileRestoredSplit(split, groups) {
  ...
  for (var i = 0; i < available.length; i++) {
    ...
    if (oldMembers && oldMembers.length === members.length
        && oldMembers.every(function (id) { return members.indexOf(id) !== -1; })) {
