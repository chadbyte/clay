...
function ensurePanel() {
  ...
  panel.setAttribute("aria-label", "Scheduled");
  panel.innerHTML =
    '<header class="scheduled-tasks-topbar">' +
      '<span class="scheduled-tasks-title">' + iconHtml("calendar-clock") + 'Scheduled</span>' +
      ...
      '<div class="scheduled-tasks-window-actions">' +
        '<button class="scheduled-tasks-icon-btn" type="button" data-action="wide" title="Widen panel" aria-label="Widen Scheduled panel" aria-pressed="false">' + iconHtml("chevrons-left-right") + '</button>' +
        '<button class="scheduled-tasks-icon-btn" type="button" data-action="fullscreen" title="Toggle fullscreen" aria-label="Toggle Scheduled fullscreen" aria-pressed="false">' + iconHtml("maximize-2") + '</button>' +
        '<button class="scheduled-tasks-icon-btn" type="button" data-action="close" title="Close" aria-label="Close Scheduled">' + iconHtml("x") + '</button>' +
      '</div>' +
    '</header>' +
    '<div class="scheduled-task-default-ai"><span>New interviews use your shared Default AI.</span><button type="button" data-action="default-ai">Change default</button></div>' +
  ...
  panel.addEventListener("change", function (event) {
    ...
    handleEditorScheduleChange(event.target, store.get('scheduledTaskTimeZone'));
  });
...
function startNewInterview() {
  ...
  if (!send({ type: "new_session", mode: "gui", forceNew: true, requestId: id, scheduleInterview: true, sourceSessionId: store.get('activeSessionId'), projectSlug: store.get('currentSlug') })) {
