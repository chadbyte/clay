import { handleIssueReference } from './clay-issue-links.js';
...
import { handleDefaultAiMessage } from './default-ai.js';
import { handleCursorSharingMessage } from './cursor-sharing-preference.js';
import { handlePendingMessageQueueMessage } from './pending-message-queue.js';

export function handleHomeProtocolMessage(msg) {
  if (handleIssueReference(msg)) return true;
...
export function processMessage(msg) {
  if (handlePendingMessageQueueMessage(msg)) return;
  if (handleDefaultAiMessage(msg)) return;
  if (handleCursorSharingMessage(msg)) return;
