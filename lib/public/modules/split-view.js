...
function handleSidebarSessionClick(event) {
  ...
  // This document capture listener runs before a row's bubble handlers. Leave
  // real controls alone so GitHub anchors retain native click, modified-click,
  // and keyboard navigation rather than selecting their owning session.
  if (event.target.closest(".session-close-btn, .session-more-btn, .session-github-link, .github-work-more")) return;
