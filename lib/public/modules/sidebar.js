...
function renderLoopGroup(loopId, children) {
  ...
  var expanded = expandedLoopGroups.has(loopId);
  var hasActive = false;
...
export function renderSessionList(sessions) {
  ...
  var loopGroups = {}; // loopId -> [sessions]
  var normalSessions = [];
  for (var i = 0; i < cachedSessions.length; i++) {
    var s = cachedSessions[i];
    if (s.loop && s.loop.loopId && s.loop.source === "ralph") {
      if (!loopGroups[s.loop.loopId]) loopGroups[s.loop.loopId] = [];
      loopGroups[s.loop.loopId].push(s);
    } else {
  ...
  var loopIds = Object.keys(loopGroups);
  for (var k = 0; k < loopIds.length; k++) {
    var lid = loopIds[k];
    var children = loopGroups[lid];
    var maxActivity = 0;
    for (var m = 0; m < children.length; m++) {
      var act = children[m].lastActivity || 0;
      if (act > maxActivity) maxActivity = act;
    }
    items.push({ type: "loop", loopId: lid, children: children, lastActivity: maxActivity });
  }
  ...
  for (var n = 0; n < items.length; n++) {
    ...
    if (item.type === "loop") {
      ctx.sessionListEl.appendChild(renderLoopGroup(item.loopId, item.children));
    } else {
