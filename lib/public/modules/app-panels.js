...
var contextData = { contextWindow: 0, maxOutputTokens: 0, model: "-", cost: 0, input: 0, output: 0, cacheRead: 0, cacheWrite: 0, turns: 0, thinking: null, costBasis: null, contextUsageReliable: true };
  ...
  store.subscribe(function (state, prev) {
    ...
    if (state.richContextUsage !== prev.richContextUsage) {
      if (state.richContextUsage) {
        // Codex exposes a lightweight snapshot for the current turn. It is
        // authoritative only when input_tokens is a valid number; a null
        // input means the adapter rejected provider accounting as context use.
        if (Object.prototype.hasOwnProperty.call(state.richContextUsage, "input_tokens")) {
          var directInput = state.richContextUsage.input_tokens;
          var directWindow = state.richContextUsage.contextWindow;
          if (typeof directWindow === "number" && directWindow > 0) contextData.contextWindow = directWindow;
          if (typeof directInput === "number" && directInput >= 0 &&
              (typeof directWindow !== "number" || directWindow <= 0 || directInput <= directWindow)) {
            contextData.input = directInput;
            contextData.contextUsageReliable = true;
          } else {
            contextData.input = 0;
            contextData.contextUsageReliable = false;
          }
        }
...
export function updateContextPanel() {
  ...
  var hasCurrentContextUsage = contextData.contextUsageReliable !== false && win > 0 && used >= 0 && used <= win;
  var pct = hasCurrentContextUsage ? (used / win) * 100 : 0;
  ...
  if (contextMiniLabel) {
    contextMiniLabel.textContent = hasCurrentContextUsage ? formatTokens(used) + "/" + formatTokens(win) : "-";
  }
  ...
  if (headerCtxEl) {
    headerCtxEl.classList.toggle("hidden", splitOpen || !hasCurrentContextUsage);
    if (!hasCurrentContextUsage) headerCtxEl.removeAttribute("data-tip");
  }
  if (contextMini) contextMini.classList.toggle("split-hidden", splitOpen);
  if (hasCurrentContextUsage && pct > 0 && !splitOpen) {
  ...
  contextUsedEl.textContent = hasCurrentContextUsage ? formatTokens(used) : "-";
  ...
  contextInputEl.textContent = hasCurrentContextUsage ? formatTokens(contextData.input) : "-";
...
export function accumulateContext(cost, usage, modelUsage, lastStreamInputTokens) {
  ...
  var contextUsageReliable = true;
  if (modelUsage) {
    var usageModels = Object.keys(modelUsage);
    for (var ui = 0; ui < usageModels.length; ui++) {
      if (modelUsage[usageModels[ui]] && modelUsage[usageModels[ui]].contextUsageVerified === false) {
        contextUsageReliable = false;
        break;
      }
    }
  }
  // Legacy Codex records did not persist a verified last-turn snapshot. Do
  // not reinterpret their billing usage as current context after replay.
  if (store.get('currentVendor') === "codex" && typeof lastStreamInputTokens !== "number") {
    contextUsageReliable = false;
  }
  contextData.contextUsageReliable = contextUsageReliable;
