...
import { handleIssueGithubClick, handleIssueGithubSubmit, handleIssueGithubInput, handleIssueGithubResult } from './issues-github.js';
...
export function requestIssue(action, args, intent) { return request(action, args, intent); }
export function refreshGithubIssue(ref) { refreshSelectedIssue(ref); }
...
export function initIssues() {
  ...
  el.addEventListener('click', function (event) {
    ...
    if (handleIssueGithubClick(target)) return;
    var entry = store.get('issuesSelected');
  ...
  el.addEventListener('submit', function (event) {
    if (handleIssueGithubSubmit(event)) return;
...
export function handleIssuesMessage(msg) {
  ...
  if (item.intent === 'github') { handleIssueGithubResult(msg, item); return true; }
