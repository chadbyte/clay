...
import { hasRecordedSpeechFor, requestRecordedSend } from './stt-recording.js';
...
export function hasSendableContent() {
  return !!(
    ...
    (ctx && hasRecordedSpeechFor(ctx.inputEl))
  );
}
...
store.subscribe(function (state, previous) {
  if ((state.speechActive === previous.speechActive && state.speechStatuses === previous.speechStatuses) || !ctx || !ctx.processing || !ctx.setSendBtnMode) return;
...
export function sendMessage() {
  if (ctx && requestRecordedSend(ctx.inputEl, sendMessage)) return;
  stopSTT();
  ...
  if (!prepareAutonomousPayload(payload)) return;
  ctx.currentMsgTs = Date.now();
  sendAcknowledgedMessage(payload);
...
export function handleAcceptedOrdinaryMessage() {
  if (!ctx) return;
  commitVendorForTurn();
...
export function initInput(_ctx) {
  ...
  document.addEventListener("paste", function (e) {
    ...
    var target = e.target;
    if (target && target.closest && target.closest(".sticky-note, #notes-archive, .confirm-modal, #paste-modal, .scheduler-detail, #debate-modal, .us-modal")) return;
