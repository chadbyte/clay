...
async function upload(active, blob) {
  ...
  setSpeechStatus(active.id, active.autoSend ? 'Transcribing before sending…' : 'Transcribing…');
  ...
  try {
    ...
    if (!currentSpeech(active)) return;
    var transcript = typeof result.text === 'string' ? result.text : '';
    var accepted = writeSpeech(active, transcript);
    stopSTT(transcript ? (active.autoSend ? 'Sending transcribed voice input…' : 'Review your draft before sending.') : 'No speech detected. Try recording again.');
    return !!(accepted && transcript.trim());
  } catch (error) {
    if (!currentSpeech(active)) return;
...
export async function finishRecording() {
  ...
  if (!active || !active.model) { stopSTT(); return false; }
  if (active.phase !== 'recording') return false;
  ...
  if (active.model.mode === 'live') {
    ...
    return upload(active, blob);
  }
}
export function hasRecordedSpeechFor(input) {
  var active = store.get('speechActive');
  return !!(active && active.input === input && active.model && active.model.mode === 'recorded' &&
    (active.phase === 'recording' || active.autoSend));
}
export function requestRecordedSend(input, send) {
  var active = store.get('speechActive');
  if (!active || active.input !== input || !active.model || active.model.mode !== 'recorded') return false;
  if (active.autoSend) return true;
  if (active.phase !== 'recording') return false;
  active.autoSend = true;
  store.set({ speechActive: active });
  setSpeechStatus(active.id, 'Transcribing before sending…');
  finishRecording().then(function (sendable) {
    if (!sendable || !active.autoSend || active.context !== speechContext() ||
        !usableSpeechInput(active.input) || active.input.value !== active.lastValue || store.get('speechActive')) return;
    active.autoSend = false;
    send();
  }, function (error) {
    if (!currentSpeech(active)) return;
    failSTT(error && error.message ? error.message : 'Voice input could not finish. Please try again.');
  });
  return true;
}
