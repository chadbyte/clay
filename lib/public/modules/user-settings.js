...
import { saveSpeechLanguage, speechLanguages } from './stt-settings.js';
...
export function initUserSettings(appCtx) {
  window.addEventListener('clay:speech-language-saved', function (event) {
    var select = document.getElementById('us-lang-select');
  ...
  if (langSelect) {
    ...
    speechLanguages.forEach(function (language) {
      var option = document.createElement('option'); option.value = language[0]; option.textContent = language[1]; langSelect.appendChild(option);
    });
    langSelect.addEventListener('change', function () {
      var select = this; select.disabled = true;
      saveSpeechLanguage(select.value).catch(function () {
        select.value = store.get('speechLang') || 'en-US'; showToast('Could not save voice language');
...
function populateAccount() {
  fetch('/api/profile').then(function (r) {
    ...
    if (langSelect) {
      langSelect.value = store.get('speechLang') || 'en-US';
    }
