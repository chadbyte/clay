...
export function initFileBrowser(_ctx) {
  ctx = _ctx;
  registerClayFileLinkOpener(openFile);
...
export function resetFileBrowser() {
  ...
  fileAtCache = {};
  pendingFileAt = null;
  store.set({ fileReadRequest: { requestId: "reset-" + Date.now() + "-" + Math.random().toString(36).slice(2), path: null }, pendingFileNavigation: null });
  // Clear tree UI
...
function requestFileContent(filePath) {
  ...
  if (ctx.ws && ctx.connected) {
    ctx.ws.send(JSON.stringify({ type: "fs_read", path: filePath, requestId: requestId, projectSlug: state.currentSlug || null, sessionId: state.activeSessionId != null ? String(state.activeSessionId) : null, accountId: state.myUserId || null }));
  }
}
...
export function handleFsRead(msg) {
  var request = store.get('fileReadRequest');
  var state = store.snap();
  if (!request || !state.connected || msg.requestId !== request.requestId || msg.path !== request.path) return;
  if ((request.projectSlug || null) !== (state.currentSlug || null)) return;
  if ((request.sessionId || null) !== (state.activeSessionId == null ? null : String(state.activeSessionId))) return;
  if ((request.accountId || null) !== (state.myUserId || null)) return;
...
function showFileContent(msg) {
  ...
  if (msg.error) {
    bodyEl.innerHTML = '<div class="file-tree-error">Unable to open <strong>' + escapeHtml(msg.path) + '</strong>: ' + escapeHtml(msg.error) + '</div>';
  } else if (msg.binary) {
    if (msg.imageUrl) {
  ...
  var pendingFileLine = store.get('pendingFileNavigation');
  var activeRead = store.get('fileReadRequest');
  if (pendingFileLine && activeRead && pendingFileLine.requestId === activeRead.requestId && pendingFileLine.path === msg.path && !msg.error) {
    var requestedLine = pendingFileLine.line;
    store.set({ pendingFileNavigation: null });
    requestAnimationFrame(function () {
      var rows = document.querySelectorAll("#file-viewer-body .file-viewer-gutter, #file-viewer-body .file-viewer-code-content");
      if (!rows.length) return;
      var target = rows[0];
      var lineHeight = parseFloat(getComputedStyle(target).lineHeight) || 20;
      document.getElementById("file-viewer-body").scrollTop = Math.max(0, (requestedLine - 1) * lineHeight - 80);
    });
  } else if (msg.error) {
    store.set({ pendingFileNavigation: null });
  }
