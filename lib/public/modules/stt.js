...
function bindComposer(id, inputId, wrapperId) {
  ...
  function render() {
    ...
    var signature = [own, pending, provider && provider.id].join(':');
    if (button.dataset.speechIcon !== signature) {
      ...
      if (provider && !own) {
        var badge = document.createElement('img'); badge.className = 'stt-vendor-badge'; badge.src = provider.icon; badge.alt = ''; button.appendChild(badge);
      }
      refreshIcons();
    }
  ...
  store.subscribe(function (state, previous) {
    var failure = (state.speechErrors || {})[id];
    if (failure && failure !== (previous.speechErrors || {})[id]) showToast(failure.message, 'warn');
  });
