...
function bindComposer(id, inputId, wrapperId) {
  ...
  function render() {
    ...
    var autoSend = own && !!active.autoSend;
    composer.classList.toggle('stt-recording', own);
    row.classList.toggle('stt-auto-send', autoSend);
    ...
    var action = own ? (autoSend ? 'Cancel pending voice send' : (pending ? 'Cancel voice input' : (model.mode === 'recorded' ? 'Stop and transcribe' : 'Stop voice input'))) : 'Start voice input';
