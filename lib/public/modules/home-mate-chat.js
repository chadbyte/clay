...
function submitMessage() {
  if (requestRecordedSend(inputEl, submitMessage)) return true;
  stopSTT();
...
function bindComposer() {
  ...
  inputEl.addEventListener("input", function () {
    resizeInput();
    sendBtn.disabled = streaming || hasPendingHomeDebateQuestion(messages) || !store.get('homeChatMateId') || !hasCommittedSessionModel() || (!inputEl.value.trim() && !hasRecordedSpeechFor(inputEl));
  });
...
store.subscribe(function (state, previous) {
  if ((state.speechActive === previous.speechActive && state.speechStatuses === previous.speechStatuses) || !inputEl || !sendBtn) return;
  renderHomeChat();
});
function ensureDom() {
...
export function renderHomeChat() {
  ...
  if (debateLaunching || mateCreationActive || !!debatePhase || (!inputEl.value.trim() && !hasRecordedSpeechFor(inputEl))) sendBtn.disabled = true;
  renderSessionModel();
