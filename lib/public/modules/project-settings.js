    ...
    for (var i = 0; i < models.length; i++) {
      (function (m) {
        ...
        item.addEventListener("click", function () {
          if (ctx.ws && ctx.connected) {
            ctx.ws.send(JSON.stringify({ type: "set_model", model: m.value }));
          }
  ...
  if (modeList) {
    ...
    for (var k = 0; k < MODE_OPTIONS.length; k++) {
      (function (opt) {
        ...
        item.addEventListener("click", function () {
          if (ctx.ws && ctx.connected) {
            ctx.ws.send(JSON.stringify({ type: "set_permission_mode", mode: opt.value }));
          }
    ...
    for (var e = 0; e < EFFORT_LEVELS.length; e++) {
      (function (lvl) {
        ...
        btn.addEventListener("click", function () {
          if (ctx.ws && ctx.connected) {
            ctx.ws.send(JSON.stringify({ type: "set_effort", effort: lvl.value }));
          }
