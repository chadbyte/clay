...
export function syncPairChrome(host, split) {
  ...
  for (var paneIndex = 0; paneIndex < paneEls.length; paneIndex++) {
    ...
    paneEls[paneIndex].style.removeProperty("--split-pane-order");
    paneEls[paneIndex].style.removeProperty("--split-worker-row");
  }
  ...
  for (var pi = 0; pi < split.panes.length && pi < headers.length; pi++) {
    var desiredPane = split.panes[pi];
    var desiredPaneEl = null;
    for (var pei = 0; pei < paneEls.length; pei++) {
      if (paneEls[pei].dataset.sessionId === String(desiredPane.sessionId)) desiredPaneEl = paneEls[pei];
    }
    if (!desiredPaneEl) continue;
    (function (sessionId, header, paneEl) {
      var isDriver = sessionId === roles.driverId;
      if (paneEl) {
        paneEl.classList.add(isDriver ? "split-pane-role-driver" : "split-pane-role-worker");
        paneEl.style.setProperty("--split-pane-order", isDriver ? "2" : String(workerIndex));
        if (!isDriver) paneEl.style.setProperty("--split-worker-row", String(workerIndex + 1));
      }
    ...
    })(desiredPane.sessionId, desiredPaneEl.querySelector(".split-pane-header"), desiredPaneEl);
  }
  ...
  for (var si = 0; si < split.panes.length; si++) {
    var livePane = null;
    for (var lpi = 0; lpi < paneEls.length; lpi++) {
      if (paneEls[lpi].dataset.sessionId === String(split.panes[si].sessionId)) livePane = paneEls[lpi];
    }
    if (!livePane) continue;
    var live = document.createElement("span");
    live.className = "split-pair-live";
    live.textContent = split.panes[si].sessionId === active.to ? "Following direction" : "Waiting";
    livePane.querySelector(".split-pane-header").appendChild(live);
  }
}
