...
export function readInterviewEngine(panel) {
  var vendor = panel && panel.querySelector('[data-engine="vendor"]');
  var model = panel && panel.querySelector('[data-engine="model"]');
  var effort = panel && panel.querySelector('[data-engine="effort"]');
  return { vendor: vendor && vendor.value || "", model: model && model.value || "", effort: effort && effort.value || "" };
}

export function renderInterviewEngine(panel, state, draft) {
  if (!panel || !state) return null;
  var vendorSelect = panel.querySelector('[data-engine="vendor"]');
  var modelSelect = panel.querySelector('[data-engine="model"]');
  var effortSelect = panel.querySelector('[data-engine="effort"]');
  var installed = state.installedVendors || [];
  var preferred = state.preference || {};
  draft = draft || {};
  var vendor = installed.indexOf(draft.vendor) !== -1 ? draft.vendor : (installed.indexOf(preferred.vendor) !== -1 ? preferred.vendor : installed[0] || "");
  vendorSelect.innerHTML = "";
  for (var i = 0; i < installed.length; i++) {
    var vendorOption = document.createElement("option");
    vendorOption.value = installed[i]; vendorOption.textContent = installed[i].charAt(0).toUpperCase() + installed[i].slice(1); vendorSelect.appendChild(vendorOption);
  }
  vendorSelect.value = vendor;
  var ready = !!(state.catalogReadyByVendor && state.catalogReadyByVendor[vendor]);
  var models = state.modelsByVendor && state.modelsByVendor[vendor] || [];
  var model = draft.vendor === vendor ? draft.model || "" : (preferred.vendor === vendor ? preferred.model || "" : "");
  modelSelect.innerHTML = "";
  if (!ready) {
    var loadingOption = document.createElement("option"); loadingOption.value = ""; loadingOption.textContent = "Models loading or unavailable"; modelSelect.appendChild(loadingOption); modelSelect.disabled = true;
  } else {
    var defaultOption = document.createElement("option"); defaultOption.value = ""; defaultOption.textContent = "Provider default"; modelSelect.appendChild(defaultOption); modelSelect.disabled = false;
    var found = !model;
    for (var mi = 0; mi < models.length; mi++) {
      var option = document.createElement("option"); option.value = modelValue(models[mi]); option.textContent = modelLabel(models[mi]); modelSelect.appendChild(option);
      if (option.value === model) found = true;
    }
    if (!found) { var staleOption = document.createElement("option"); staleOption.value = model; staleOption.textContent = "Unavailable: " + model; staleOption.disabled = true; modelSelect.appendChild(staleOption); }
    modelSelect.value = model;
  }
  var levels = effortLevelsFor(vendor, models, model);
  var effort = draft.vendor === vendor && draft.model === model ? draft.effort || "" : (preferred.vendor === vendor && preferred.model === model ? preferred.effort || "" : "");
  ...
  effortSelect.value = levels.indexOf(effort) !== -1 ? effort : "";
  effortSelect.disabled = !ready;
  var selection = { vendor: vendor, model: model, effort: effortSelect.value, catalogReady: ready, valid: ready && installed.indexOf(vendor) !== -1 && (!model || modelSelect.selectedIndex !== -1 && !modelSelect.options[modelSelect.selectedIndex].disabled) };
  var start = panel.querySelector('[data-action="new"]');
  if (start) { start.disabled = !selection.valid; start.title = ready ? (selection.valid ? "" : "Choose an available interview model") : "Interview models are still loading or unavailable"; }
  return selection;
}
