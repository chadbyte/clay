...
import { browserSpeechInfo, compareSpeechModels } from './stt-browser.js';
...
function render() {
  ...
  if (settings) {
    ...
    settings.models.filter(function (model) { return model.mode === state.mode; }).sort(compareSpeechModels).forEach(function (model) {
      ...
      var unavailable = model.provider === 'browser' && !browser.available;
      var mark = provider ? '<img src="' + provider.icon + '" alt="">' : browser.icon ? '<img src="' + browser.icon + '" alt="">' : '<span class="stt-browser-icon">' + iconHtml('globe') + '</span>';
      var label = provider ? provider.name + ' · ' + model.name : model.name + (browser.id === 'browser' ? '' : ' · ' + browser.name);
      html += '<div class="stt-model-row"><button type="button" class="stt-model" data-model="' + model.id + '" data-focus="' + model.id + '" title="' + escapeHtml(label + ' — ' + (unavailable ? browser.reason : model.description)) + '" aria-label="' + escapeHtml(label) + '" aria-pressed="' + (model.id === settings.selected) + '"' + (unavailable ? ' disabled aria-describedby="stt-browser-unavailable"' : '') + '>' + mark + '<span>' + escapeHtml(label) + '</span>' + (model.id === settings.selected ? '<span class="stt-model-check">' + iconHtml('check') + '</span>' : '') + '</button>';
      ...
      html += '</div>';
      if (unavailable) html += '<p id="stt-browser-unavailable" class="stt-browser-unavailable">' + escapeHtml(browser.reason) + '</p>';
