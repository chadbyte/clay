...
> Automatic Issues cover defects evidenced on the default or established release/integration baseline and independent durable project backlog. Current-edit failures and task-local deferrals stay in the active task and its Log; unknown baseline impact does not justify automatic filing. Explicit user tracking requests may include branch-specific work. `issues-filing-guidance.js` shares filing and short Issue-link note policies between Issue tools, Driver attention guidance, and note tools. Authorized Drivers search/reuse before creation and record baseline evidence, impact, next action, acceptance criteria, remediation, and verification.
...
| `project-session-delete.js` | `delete_session` and `bulk_delete_sessions` scope and authorization. Explicit human deletion of a Driver also deletes every Split Worker generation (current and replaced) that the provenance hierarchy links to it: a unique parent origin, the same owner, and a parent that is not itself a Worker. Ambiguous and cross-owner links are never expanded. The caller's delete permission and access are checked for the whole scope before anything happens. If any cascaded Worker is inaccessible, the request is refused. PTYs and title watchers are released, then the existing session-manager deletion cleanup runs. Deleting a Worker affects only that Worker; close, dissolve, and replacement never come through here and keep history |
| `project-filesystem.js` + `project-file-history.js` + `project-file-path.js` + `project-file-http.js` | `fs_list`, `fs_read`, `fs_write`, `fs_watch`, `fs_unwatch`, `fs_file_history`, `fs_git_diff`, `fs_file_at`, `get_project_env`, `set_project_env`, `read_global_claude_md`, `write_global_claude_md`, `get_shared_env`, `set_shared_env` | File browser, file history, project env/settings. `project-request-access.js` supplies fresh user-bound file scope across WS, HTTP download/image routes and watches: personal and mapped OS identities use OS permissions; unmapped multi-user access stays project-bound for every role. `project-file-path.js` resolves scope and distinguishes missing paths, scope denial and OS denial; Git history remains repository-scoped. |
...
| `project-scheduled-tasks.js` + `scheduled-task-records.js` + `schedule-validation.js` + `record-revision.js` | `scheduled_tasks_list`, `scheduled_task_interview_*`, `scheduled_task_manual_start`, `scheduled_task_create`, `scheduled_task_update`; query-bound schedule create/list/read/update tools | Project Driver Scheduled Tasks workbench lifecycle, one-question interviews using the user's shared Default AI, exact session/query/draft correlation, owner/revision authorization, shared validated record updates with monotonic persisted revisions, strict cron and one-off validation, and no-query manual creation. Ownerless migrated records are visible and claimable only by an authorized administrator or project owner through the explicit `claimOwner` update. `sdk-bridge.js` delivers query-bound tools through Claude SDK MCP, Codex dynamic tools, or the exact-query ACP/Kiro stdio bridge |
| `project-scheduled-task-execution.js` + `scheduled-run-state.js` + `project-scheduled-result-store.js` | `scheduled_task_run`, `scheduled_task_open_run`; query-bound `report_scheduled_task_outcome` | Owner-bound Scheduled Tasks execution through a fresh hidden Driver/Split Worker pair for each occurrence. Persists active and terminal run identity before acknowledgement, records idempotent server-authored terminal results in Logs, revalidates the stored owner, live project access, and required OS identity immediately before SDK dispatch, recovers ambiguous restarts as interrupted, and routes due and Run now requests through the same executor. When `skipIfRunning` is false, overlapping starts coalesce into one durable `pendingRun`; additional overlaps do not create an unbounded queue |
| `session-query-tool-bridge.js` | Exact-query projection and dispatch for SDK-owned Scheduled Tasks and structured-input handlers exposed through the project stdio MCP endpoint |
| `project-shell-command.js` | `shell_command` | One-shot composer shell execution and pending agent context capture |
| `project-loop.js` | `loop_start`, `loop_stop`, legacy `ralph_*`, `loop_registry_*`, `schedule_create`, `hub_schedules_list`, `delete_loop_group` | Compatibility execution for existing Loop jobs, stored prompt files, history, registry, and scheduling; new interactive tasks use Until complete. In multi-user mode every legacy coder and judge SDK start revalidates the authoritative record owner, live project authorization, session ownership, and required OS identity; single-user standalone Loops retain their existing execution path |
...
| `project-autonomous-run.js` + `autonomous-run-record.js` + `autonomous-run-scheduler.js` + `project-full-access.js` | `autonomous_run_arm`, `autonomous_run_disarm`, `autonomous_run_stop`, `autonomous_run_resume`; `report_until_complete_outcome` MCP tool | Server-owned Until complete lifecycle. Deadline, rate-limit, and immediate-continuation wakes use the shared durable scheduler with persisted session origin, run/epoch, and monotonic wake sequence. Restarts pause without automatic execution; explicit resume, Stop cancellation, exact-query rejection guards, structured Driver assessment, limits, and serialized Skip Permissions restore remain authoritative |
...
| `server-default-ai.js` | `default_ai_get`, `default_ai_catalog_get`, `default_ai_set` | Authenticated user-island Default AI preference/catalog service and same-user broadcast; resolves fresh builtin Clay, Scheduled interview, and internal AI helper runtimes without changing existing conversations or custom Mate runtimes |
...
| `scheduler.js` + `loop-cron.js` + `loop-registry-occurrence.js` + `loop-durable-schedule.js` | Legacy-compatible Loop and Scheduled Task record metadata plus durable recurrence adapter. The registry preserves legacy IDs/files, anchored rules, cron, counters, run reservations, and explicit owner-setup state; the adapter gives each occurrence a stable schedule identity plus mutation epoch, reconciles interrupted ledger work against registry state, applies overdue cutoffs before enqueue, and registers one project handler on the server-owned scheduler. Public metadata mutations roll back if durable synchronization cannot persist |
| `notes.js` + `notes-lifecycle.js` | Sticky Note storage and its reversible lifecycle. A note is `open` or `closed`; completing one **closes** it, recording `closedAt` and a server-derived `closedBy` actor. Human UI permanent deletion is a separate authenticated action restricted to currently closed notes; agent/MCP removal remains a reversible close. Legacy notes project as open, except the older `hidden: true` flag which already meant the same thing and projects as closed with no invented timestamp. Projection runs on load, so reading rewrites nothing. `remove()` remains maintenance-only; `removeClosed()` is the guarded storage operation for the human delete path |
...
| `project-request-access.js` | Current-user authorization for project message routing, exact environment targets, administrator-only global settings, session history visibility, and fail-closed OS identity resolution |
| `project-file-watch.js` | Per-socket file and directory subscriptions; rechecks project/file permissions and paths on updates, reads under requester OS identity, and cleans up subscriptions on disconnect |
| `project-session-spawn.js` | Agent-driven sibling session creation, safety policy, and concurrency queue |
| `project-models.js` + `project-tool-llm-runtime.js` | Vendor model discovery, loading/error responses, model matching, selection acknowledgements, and actor-bound Capsule LLM execution |
| `model-entry.js` | Dependency-free canonical model value/match helpers shared by server-side catalog resolvers |
| `default-ai-runtime.js` | Clay-owned Default AI preference/runtime resolution, vendor-specific model ranking, catalog validation, and reasoning-effort fallback |
...
| `session-title-generator.js` + `yoke/title-query.js` | Bounded session title requests and cooldown recovery; shared Claude/Codex collector requires successful terminal results, rejects provider failures, propagates cancellation and closes one-shot handles. |
| `session-provenance.js` | Durable Driver/Split Worker creation provenance. Assigns opaque per-session origin IDs, records the originating Driver and monotonic Worker generation at pair-factory creation, restores legacy metadata without guessing from titles or split state, and produces owner-safe hierarchy projections. `ownedWorkersByDriver` is the single definition of the Workers a Driver deletion cascades to, used by both the delete handler and the session list's `ownedWorkerCount`. Missing, deleted, inaccessible, corrupt, or ambiguous parents leave a session classified as an orphaned Worker rather than promoting it to Driver |
| `session-split-groups.js` + `session-split-group-cleanup.js` + `session-split-group-recovery.js` | Split group store and its deletion recovery. Reversible structural mutations roll back when persistence fails. Session deletion is irreversible: `session-split-group-cleanup.js` applies it to live membership first, never restores the deleted session, notifies pair/UI consumers, logs the failed write, and retries with bounded backoff until any successful save writes the live state (`shutdown()` cancels the retry on project destroy). On restart before that retry, `session-split-group-recovery.js` reduces a stale Version 2 record to its surviving Driver and Workers only when member and pair anchors agree, survivors match exactly with the group owner, and removed anchors match no live session at all. It also requires that the session manager read every session record: after any unreadable or unparseable record (`sm.sessionLoadUncertain()`), absence is not treated as deletion. Otherwise the record is preserved |
...
| `project-driver-continuation.js` + `driver-continuation-trigger.js` | Human-reviewed Project Driver continuation lifecycle plus conservative proactive trigger policy. The server accepts at most one durable proactive card per source session and requires current measured context pressure or a completed recorded compaction, a completed milestone, a concrete benefit, a prepared next action, and an otherwise safe source. Unknown or cumulative usage and model-supplied evidence never qualify; Stay here permanently suppresses proactive suggestions for that source. |
...
| `project-session-pair.js` + `session-pair-query-start.js` | **Coordinator** for the visible Driver/Split Worker pair. Owns delegation (`send_to_partner`), `read_partner`, interrupt/close, result push-back, and wiring for task controls. Exact live-session identity gates every pair action; a detached result's lazy query-start failure may clear processing only while its exact session, query generation, and resume token still own the start. Delegates task state, history projection, creation, lifecycle, and prompts to bounded modules below |
...
| `project-pair-replacement-state.js` | Idempotent runtime-replacement transactions with source/target generation correlation, stages, terminal failure details, and rollback outcome |
| `session-pair-history.js` | Bounded response, error, and recent-turn extraction shared by pair delegation and result delivery |
| `session-pair-operation.js` | Stable payload fingerprints for replay-safe pair operation ids; an id reused with altered delegation or replacement input fails closed |
...
| `session-pair-mcp-server.js` | SDK-free base pair tool definitions. Lifecycle pairs add live messaging, task queue controls, correlated resume/replace operations, status, runtime replacement proposals, and generation evaluation through coordinator wiring |
...
| `project-worker-proposal.js` | Split Worker creation and replacement runtime decisions. Proposals have stable creation/status data and exact cancel/supersede semantics; stale resolved cards cannot mutate. Auto and manual acceptance share exact-session, owner, pair, catalog, and transaction checks. `worker-runtime-catalog.js` projects executable model/effort combinations and explicit unavailable/approval reasons |
| `project-driver-continuation.js` + `driver-continuation-access.js` + `driver-continuation-transaction.js` + `driver-continuation-lease.js` + `driver-continuation-startup.js` | Session-bound Driver continuation proposals, authoritative live actor/project access, exact-owner approval, safe-boundary blockers, guarded compact successor startup, two-sided durable transition state, stable source relationships, and idempotent retry. Attached Split Worker migration is deliberately blocked until pair ownership can move transactionally |
...
| `sdk-skill-discovery.js` | SDK compatibility wrapper for shell segment splitting and shared skill inventory access; cross-vendor roots and metadata belong to `yoke/skill-discovery.js` |
...
| `sdk-bridge.js` → `checkToolWhitelist` | Where a tool becomes auto-approved. The non-mutating `propose_worker` call, pair follow-up/lifecycle tools, and `respond_to_worker_permission` are allowed here so provider authorization never replaces or duplicates the separate human runtime-selection card. Reversible Sticky Note operations are also allowed under the exact `mcp__clay-notes__` namespace; their session-bound handlers still enforce authorization and lifecycle rules. Live authorized Project Drivers may close any project note; other sessions may close only their own, and reopening remains creator-only. Each handler re-checks identity, eligibility, owner and live pair as applicable. Accept only name forms the bridge can genuinely emit and bind namespace checks exactly so another server cannot borrow a safe tool name. `spawn_sessions` is deliberately excluded. When adding an entry, add exact names rather than a permissive suffix match |
...
| `users-default-ai-preferences.js` | Strict per-user/single-user Default AI preference persistence and legacy Scheduled interview preference reads |
...
| `yoke/skill-discovery.js` | Unified cross-vendor skill inventory: scoped user/project roots, precedence, canonical deduplication, frontmatter metadata, and authorized referenced-content reads |
| `yoke/session-skill-catalog.js` | Session-scoped skill and vendor slash-command hydration; refreshes personal catalogs without broadcasting them across users or sessions |
| `context-usage.js` + `yoke/claude-context-usage.js` | Ordered summary reads and authorized on-demand full context detail, including correlated isolated Worker requests |
| `sdk-turn-queue.js` | Reconciles provider queue counts with later local submissions and legacy merged-message accounting |
| `yoke/claude-model-switch.js` | PostModelSwitch hook composition for recording confirmed model changes in session history |
| `yoke/claude-message-correlation.js` | Query-local UUID tracking for Claude input and merged result accounting; shared by in-process and isolated Worker handles |
...
| `yoke/mcp-bridge-server.js` | Stdio MCP server spawned by Codex, shared ACP agents, and Kiro. Proxies tool list/call to Clay via HTTP at `/api/mcp-bridge`; session-only instances carry exact query generation binding |
| `yoke/session-tool-transport.js` | Shared capability and canonical tool-name mapping for Claude SDK MCP, Codex dynamic tools, ACP/Kiro stdio MCP, and unsupported runtimes |
...
| `driver-continuation.js` + `driver-continuation-state.js` | Inline Driver continuation review card with project/source/proposal/request-correlated lifecycle state in the shared store. Requires an explicit human choice, recovers from send failure, timeout, or disconnect, restores server-authoritative transcript state on replay, rejects stale responses, and renders source-link failures in the successor |
| `app-loop-ui.js` | Compatibility UI for existing scheduled Loop status, preview, and execution history |
| `autonomous-run.js` | Project Driver composer mode selector and synchronized Until complete status/details/Stop/Resume UI; mutable state lives in the shared store |
...
| `session-delete-scope.js` | Pure client description of the server deletion scope for single-session and group-clear confirmations. The total of Workers a Driver deletion also removes comes from the server's `ownedWorkerCount` in `session_list`, counted over every session including hidden ones and replaced generations. In multi-user mode it is sent only to the Driver's owner (trusted socket identity, on both connect and broadcast), and omitted, never 0, for everyone else. The visible list never stands in for it. When that total is unknown (session missing from the cache, or no count), the wording is conservative and group totals say "at least". Any deletion that removes or may remove Workers uses the custom confirmation dialog instead of the two-click armed shortcut. Worker and plain-session wording is unchanged |
...
| `scheduled-tasks.js` + `scheduled-task-form.js` + `scheduled-task-engine.js` + `scheduled-task-edit-state.js` + `scheduled-task-presentation.js` | Project-only Scheduled Tasks right workbench, calendar-style once/recurring form with advanced cron and explicit execution runtime controls, shared Default AI interview entry, correlated create/edit state with dirty-form conflict preservation, and direct coordination with the existing right-window modules. Active `needs-input` runs remain labelled as blocked and expose their exact hidden Driver through Open blocked run; the legacy calendar remains in `scheduler.js` |
...
| `project-logs.js` + `project-logs-render.js` + `scheduled-result-client.js` + `scheduled-result-display.js` + `scheduled-result-notification.js` | Project Logs ledger in the bounded right workbench pane. One navigation stack: full-pane list, full-pane entry detail, Back preserving query/filter/scroll. `project-logs-render.js` owns markup only (ledger rows, detail, discussion) and never touches the WebSocket. `scheduled-result-client.js` contains pure result-news decisions; `scheduled-result-display.js` directly owns visible-document acknowledgement, accessible heading focus, and exact hidden-session requests; `scheduled-result-notification.js` owns the top-right live-result banner. There is no canonical create or edit UI: people read and comment. Logs open from the explicit tool control or an exact rendered `log:` reference. Canonical agent updates retain live-session badges; Scheduled Tasks results use server-persisted per-user unread state, reconnect focus, live completion banners, and exact hidden-session navigation |
...
| `permission-hints.js` | Shared permission dialog/banner presentation: provider-requested decline focus and suppression of persistent approval controls |
