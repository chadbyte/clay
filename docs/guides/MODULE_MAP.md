...
| `project-scheduled-task-execution.js` + `scheduled-run-state.js` | `scheduled_task_run`, `scheduled_task_open_run`; query-bound `report_scheduled_task_outcome` | Owner-bound Scheduled Tasks execution through a fresh visible Driver/Split Worker pair for each occurrence. Persists active and terminal run identity before acknowledgement, revalidates the stored owner, live project access, and required OS identity immediately before SDK dispatch, recovers ambiguous restarts as interrupted, and routes due and Run now requests through the same executor. When `skipIfRunning` is false, overlapping starts coalesce into one durable `pendingRun`; additional overlaps do not create an unbounded queue |
