...
| `project-scheduled-tasks.js` + `schedule-validation.js` | `scheduled_tasks_list`, `scheduled_task_interview_*`, `scheduled_task_manual_start`, `scheduled_task_create`, `scheduled_task_update`; query-bound `begin_scheduled_task_interview` / `propose_scheduled_task` | Project Driver Scheduled Tasks workbench lifecycle, exact session/query/draft correlation, owner authorization, strict cron and one-off validation, agent-assisted creation, and no-query manual creation |
...
| `scheduled-tasks.js` | Project-only Scheduled Tasks right workbench, correlated create/edit state, and direct coordination with the existing right-window modules; the legacy calendar remains in `scheduler.js` |
