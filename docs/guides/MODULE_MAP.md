...
| `scheduled-tasks.js` + `scheduled-task-form.js` + `scheduled-task-engine.js` + `scheduled-task-edit-state.js` | Project-only Scheduled Tasks right workbench, calendar-style once/recurring form with advanced cron and execution controls, persisted interview-engine selection, correlated create/edit state with dirty-form conflict preservation, and direct coordination with the existing right-window modules; the legacy calendar remains in `scheduler.js` |
