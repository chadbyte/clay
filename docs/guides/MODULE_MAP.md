...
| `project-autonomous-run.js` + `project-full-access.js` | `autonomous_run_arm`, `autonomous_run_disarm`, `autonomous_run_stop`, `autonomous_run_resume`; `report_until_complete_outcome` MCP tool | Server-owned Until complete lifecycle, bounded persistence, correlated continuation, limits, restart pause/resume, structured Driver assessment, and serialized Skip Permissions restore |
